...
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Việc kích hoạt tính năng xâm nhập sẽ giúp ứng dụng vùng chứa có thể truy cập được và quy tắc chia tỷ lệ TCP có thể kích hoạt các bản sao từ các kết nối TCP đến ngay cả khi bản sửa đổi đã chia tỷ lệ về 0. Azure Container Apps ghi lại tỷ lệ TCP với `minReplicas` được đặt thành `0` và được định cấu hình xâm nhập cho lưu lượng TCP.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-6', 'az-204', 6, 'HOTSPOT –  
  
You need to fix the VM issues. Which tools should you use?', '[]'::jsonb, '[]'::jsonb, 'Azure Backup tạo các điểm khôi phục cho máy ảo Azure và khôi phục máy ảo, ổ đĩa hoặc tệp từ các bản sao lưu đó. Mạng tăng tốc cải thiện hiệu suất mạng VM thông qua ảo hóa I/O gốc đơn, giảm độ trễ, jitter và mức sử dụng CPU. Azure Network Watcher được sử dụng để giám sát và chẩn đoán mạng thay vì tự cung cấp tính năng nâng cao hiệu suất.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which command and parameter should you specify?', '[]'::jsonb, '[]'::jsonb, '`func init` tạo dự án Azure Functions mới. Tùy chọn `--worker-runtime` đặt thời gian chạy ngôn ngữ của nó, bao gồm Python và C#; Thay vào đó, `func new` tạo một hàm từ một mẫu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which consistency level should you use?', '[{"key":"A","text":"consistent prefix"},{"key":"B","text":"eventual"},{"key":"C","text":"bounded staleness"},{"key":"D","text":"strong"},{"key":"E","text":"session"}]'::jsonb, '["D"]'::jsonb, 'Tính nhất quán mạnh mẽ của Azure Cosmos DB mang lại khả năng tuyến tính hóa: các lần đọc luôn trả về phiên bản mục được cam kết gần đây nhất và các lần ghi đã cam kết được sắp xếp trên toàn cầu. Mức độ nhất quán yếu hơn có thể trả về dữ liệu cũ hoặc chỉ cung cấp đảm bảo đặt hàng trong giới hạn hoặc phiên giới hạn hơn.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure Azure CDN caching rules. Which options should you use?', '[]'::jsonb, '[]'::jsonb, 'Ghi đè thực thi thời gian tồn tại của bộ nhớ đệm CDN đã định cấu hình thay vì tôn trọng giá trị hết hạn do nguồn cung cấp, đảm bảo rằng phương tiện sẽ hết hạn sau một giờ. Bộ nhớ đệm mỗi URL duy nhất làm cho giá trị chuỗi truy vấn `quality` trở thành một phần của khóa bộ đệm, do đó, mỗi biến thể chất lượng video đều được lưu vào bộ nhớ đệm và phân phối chính xác từ các POP cạnh CDN.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to purge individual assets from the Front Door cache. Which type of cache purge should you use?', '[{"key":"A","text":"single path"},{"key":"B","text":"wildcard"},{"key":"C","text":"root domain"}]'::jsonb, '["A"]'::jsonb, 'Azure Front Door sử dụng tính năng lọc một đường dẫn để xóa nội dung được lưu trong bộ nhớ đệm riêng lẻ bằng cách chỉ định đường dẫn đầy đủ của nội dung đó, bao gồm cả phần mở rộng tệp của nội dung đó. Ký tự đại diện sẽ xóa các nhóm tệp và thư mục mục tiêu, đồng thời việc xóa tên miền gốc sẽ xóa tất cả nội dung cho điểm cuối.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the Azure CLI command?', '[]'::jsonb, '[]'::jsonb, 'Đối với vùng chứa Docker tùy chỉnh trong Azure App Service, ghi nhật ký Docker/vùng chứa phải được bật bằng `--docker-container-logging filesystem`. Sau đó, lệnh `az webapp log tail` truyền trực tuyến đầu ra nhật ký ứng dụng/vùng chứa trong thời gian thực.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which four actions should you perform in order?', '{"statements":[{"id":"step1","text":"Slot 1:","correctAnswer":"B"},{"id":"step2","text":"Slot 2:","correctAnswer":"F"},{"id":"step3","text":"Slot 3:","correctAnswer":"A"},{"id":"step4","text":"Slot 4:","correctAnswer":"D"}],"choices":[{"key":"A","text":"Generate a key transfer blob file by using the HSM vendor-provided tool."},{"key":"B","text":"Generate a Key Exchange Key (KEK)."},{"key":"C","text":"Create a custom policy definition in Azure Policy."},{"key":"D","text":"Run the az keyvault key import command."},{"key":"E","text":"Run the az keyvault key restore command."},{"key":"F","text":"Retrieve the Key Exchange Key (KEK) public key."}]}'::jsonb, '["step1=B","step2=F","step3=A","step4=D"]'::jsonb, 'KEK được tạo trong Key Vault đích sẽ bảo vệ khóa đích trong quá trình truyền. Việc xuất khóa công khai KEK cho phép công cụ BYOK của nhà cung cấp HSM mã hóa và đóng gói khóa tại chỗ dưới dạng blob chuyển; `az keyvault key import` tải blob đó lên, trong đó Key Vault giải mã và nhập nó trong ranh giới HSM của nó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to implement the policy fragment.', '{"statements":[{"id":"section_open","text":"Slot 1:","correctAnswer":"B"},{"id":"policy_name","text":"Slot 2:","correctAnswer":"F"},{"id":"attribute_name","text":"Slot 3:","correctAnswer":"E"},{"id":"section_close","text":"Slot 4:","correctAnswer":"B"}],"choices":[{"key":"A","text":"name"},{"key":"B","text":"inbound"},{"key":"C","text":"outbound"},{"key":"D","text":"set-variable"},{"key":"E","text":"fragment-id"},{"key":"F","text":"include-fragment"}]}'::jsonb, '["section_open=B","policy_name=F","attribute_name=E","section_close=B"]'::jsonb, 'Tất cả các API đều kế thừa chính sách Quản lý API toàn cầu. Phần gửi đến xử lý các lệnh gọi API đến và chính sách include-fragment sẽ chèn một đoạn có thể sử dụng lại bằng cách sử dụng thuộc tính id đoạn được yêu cầu. Do đó, APICounts được đưa vào dưới dạng <include-fragmentFragment-id="APICounts" /> trong phần gửi đến toàn cầu.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783832053632-723nhkt4.png', 'published'),
...
Which hosting model should you choose?', '[{"key":"A","text":"Premium plan"},{"key":"B","text":"App Service plan"},{"key":"C","text":"Consumption plan"}]'::jsonb, '["A"]'::jsonb, 'Azure Functions Premium hỗ trợ kết nối mạng ảo và các điểm cuối riêng tư để truy cập an toàn vào các tài nguyên được bảo vệ, trong khi vẫn duy trì khả năng mở rộng theo sự kiện. Gói Dịch vụ ứng dụng sử dụng dung lượng chuyên dụng và được tính phí ngay cả khi chức năng này không hoạt động; gói Tiêu thụ cũ là trả tiền cho mỗi lần thực thi nhưng không hỗ trợ tích hợp mạng ảo hoặc điểm cuối riêng tư. Do đó, Premium là sự cân bằng phù hợp giữa khả năng bảo mật cần thiết và khả năng mở rộng hiệu quả về mặt chi phí.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
**NOTE:** Each correct selection is worth one point.', '[{"key":"A","text":"Service Bus"},{"key":"B","text":"Event Hub"},{"key":"C","text":"Event Grid"},{"key":"D","text":"Queue"}]'::jsonb, '["A","C"]'::jsonb, 'Các chủ đề và đăng ký Azure Service Bus cung cấp tin nhắn xuất bản/đăng ký lâu dài từ một đến nhiều người. Azure Event Grid là dịch vụ đăng ký xuất bản được quản lý, có thể đẩy các sự kiện đến người đăng ký khi xảy ra thay đổi trạng thái, tránh việc bỏ phiếu liên tục. Người sử dụng Trung tâm sự kiện sử dụng mô hình kéo, trong khi hàng đợi sử dụng tính năng nhắn tin điểm-điểm.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to implement push notifications. How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, '`NotificationHubClient.CreateClientFromConnectionString` xây dựng ứng dụng khách Trung tâm thông báo từ chuỗi kết nối và tên trung tâm. `SendWindowsNativeNotificationAsync` gửi tải trọng XML thông báo có nguồn gốc từ Windows thông qua ứng dụng khách đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Access key"},{"key":"B","text":"SSL port"},{"key":"C","text":"Subscription name"},{"key":"D","text":"Location"},{"key":"E","text":"Host name"},{"key":"F","text":"Subscription id"}]'::jsonb, '["A","B","E"]'::jsonb, 'Bộ đệm ẩn Azure an toàn cho kết nối máy khách Redis yêu cầu tên máy chủ bộ đệm, cổng TLS/SSL và khóa truy cập để xác thực. Siêu dữ liệu đăng ký và vị trí là thuộc tính quản lý tài nguyên Azure, không phải cài đặt kết nối.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-35', 'az-204', 35, 'You need to implement the Azure Function for retail store locations.  
  
How should you configure the solution?', '[]'::jsonb, '[]'::jsonb, 'Trình kích hoạt lưu trữ Blob chạy khi phát hiện thấy blob mới hoặc cập nhật. Hàm phải duy trì dữ liệu lưu trữ đã được xử lý trong Azure Cosmos DB, yêu cầu liên kết đầu ra Azure Cosmos DB; ràng buộc đó ghi tài liệu thông qua API SQL.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Standard"},{"key":"B","text":"URL ping"},{"key":"C","text":"Custom testing using the TrackAvailability API method"},{"key":"D","text":"Multi-step"}]'::jsonb, '["A","C"]'::jsonb, 'Kiểm tra tính khả dụng tiêu chuẩn của Application Insights có thể xác thực tính hợp lệ của chứng chỉ TLS/SSL và gửi tiêu đề HTTP tùy chỉnh. Kiểm tra tính khả dụng tùy chỉnh có thể chạy logic kiểm tra do ứng dụng xác định và gửi kết quả của nó tới Application Insights bằng cách sử dụng `TrackAvailability()`. Kiểm tra ping URL không được dùng nữa và kiểm tra web nhiều bước không phải là loại kiểm tra tính khả dụng được hỗ trợ hiện tại.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-38', 'az-204', 38, 'You need to implement the delivery service telemetry data.  
  
How should you configure the solution?', '[]'::jsonb, '[]'::jsonb, 'API Azure Cosmos DB dành cho NoSQL hỗ trợ ngôn ngữ truy vấn SQL gốc, đáp ứng yêu cầu về truy vấn SQL. Sử dụng biển số xe làm khóa phân vùng sẽ nhóm dữ liệu từ xa của từng phương tiện đồng thời cung cấp khóa số lượng cao giúp phân phối dữ liệu hiệu quả hơn dung lượng gói hoặc tọa độ; ID mục thường sẽ tạo các phân vùng logic một mục.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which two parameters must be used together to satisfy this requirement? (Choose two.)', '[{"key":"A","text":"EnabledForDeployment"},{"key":"B","text":"EnablePurgeProtection"},{"key":"C","text":"EnabledForTemplateDeployment"},{"key":"D","text":"EnableSoftDelete"}]'::jsonb, '["B","D"]'::jsonb, 'Xóa mềm sẽ giữ lại các đối tượng Azure Key Vault đã xóa trong khoảng thời gian lưu giữ và tính năng bảo vệ thanh lọc sẽ ngăn chặn việc xóa vĩnh viễn chúng trong khoảng thời gian đó. Bảo vệ thanh lọc phụ thuộc vào xóa mềm; cài đặt triển khai và triển khai mẫu không kiểm soát việc lưu giữ đối tượng đã xóa.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which levels should you configure?', '[]'::jsonb, '[]'::jsonb, 'Azure Cosmos DB dành cho NoSQL sử dụng tính nhất quán của Phiên theo mặc định. Yêu cầu đọc có thể chỉ định mức độ nhất quán yếu hơn, do đó các lần đọc có thể sử dụng tiền tố nhất quán. Việc ghi luôn sử dụng mức nhất quán được định cấu hình cho tài khoản và không hỗ trợ ghi đè theo yêu cầu, do đó việc ghi sử dụng tính nhất quán của Phiên.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-42', 'az-204', 42, 'Solution: Use Blob storage events to trigger photo processing.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Các sự kiện Azure Blob Storage được phân phối thông qua Azure Event Grid để xử lý theo hướng sự kiện gần như theo thời gian thực. Tài khoản lưu trữ v2 đa năng hỗ trợ tích hợp Lưới sự kiện và xử lý hình ảnh là một kịch bản sự kiện Blob Storage được ghi lại.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-43', 'az-204', 43, 'Solution: Change the Azure Storage account to a BlockBlobStorage storage account.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Việc thay đổi tài khoản General-Purpose v2 thành tài khoản BlockBlobStorage không cung cấp cơ chế phát hiện ảnh tải lên và bắt đầu xử lý hình ảnh trong vòng một phút. Các loại tài khoản Azure Storage cũng không thể thay đổi thành loại khác; chuyển sang loại khác yêu cầu tạo tài khoản mới và sao chép dữ liệu. [Azure Storage account overview](https://learn.microsoft.com/en-us/azure/storage/common/storage-account-overview)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Trình kích hoạt Azure Blob Storage tiêu chuẩn sử dụng tính năng bỏ phiếu và không đảm bảo rằng quá trình xử lý sẽ bắt đầu trong vòng một phút sau khi tải lên blob. Cần có trình kích hoạt dựa trên sự kiện có độ trễ thấp hơn, chẳng hạn như trình kích hoạt Blob dựa trên Lưới sự kiện, để đáp ứng yêu cầu bắt đầu dưới phút.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Trình kích hoạt Blob Storage dựa trên cuộc bỏ phiếu mặc định trên gói Tiêu thụ Azure Functions có thể trì hoãn việc xử lý blob mới tối đa 10 phút sau khi ứng dụng chức năng không hoạt động. Do đó, nó không thể đảm bảo rằng quá trình xử lý ảnh sẽ bắt đầu trong vòng một phút; trình kích hoạt Blob dựa trên sự kiện sử dụng Lưới sự kiện mang lại độ trễ thấp hơn.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-46', 'az-204', 46, 'Solution: Use the Azure Blob Storage change feed to initiate photo processing.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Bản ghi nguồn cấp dữ liệu thay đổi Azure Blob Storage sẽ có sẵn trong vòng vài phút sau khi thay đổi, thay vì cung cấp trình kích hoạt dưới phút. Do đó, nó không thể đảm bảo quá trình xử lý ảnh bắt đầu trong vòng chưa đầy một phút.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
To which policy section should you add the policies? Each section may be used once, more than once, or not at all.', '{"statements":[{"id":"set-variable","text":"Set-variable","correctAnswer":"A"},{"id":"cache-lookup-value","text":"Cache-lookup-value","correctAnswer":"A"},{"id":"cache-store-value","text":"Cache-store-value","correctAnswer":"B"},{"id":"find-and-replace","text":"Find-and-replace","correctAnswer":"B"}],"choices":[{"key":"A","text":"Inbound"},{"key":"B","text":"Outbound"}]}'::jsonb, '["set-variable=A","cache-lookup-value=A","cache-store-value=B","find-and-replace=B"]'::jsonb, '`set-variable` trích xuất và lưu danh tính JWT trước khi xử lý phụ trợ và `cache-lookup-value` kiểm tra giá trị được lưu trong bộ nhớ đệm dành riêng cho người dùng theo yêu cầu gửi đến. `cache-store-value` lưu trữ kết quả sau khi có sẵn và `find-and-replace` sửa đổi nội dung phản hồi; cả hai đều thuộc phần outbound.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you use?', '[{"key":"A","text":"Azure BlobFuse"},{"key":"B","text":"Azure Disks"},{"key":"C","text":"Azure Storage Network File System (NFS) 3.0 support"},{"key":"D","text":"Azure Files"}]'::jsonb, '["A"]'::jsonb, 'Azure BlobFuse gắn bộ chứa Azure Blob Storage dưới dạng hệ thống tệp trong Linux và chuyển các hoạt động của hệ thống tệp thành lệnh gọi API Azure Blob REST. Điều này cho phép ứng dụng Linux không thay đổi hoạt động với kho lưu trữ hình ảnh dựa trên blob qua HTTPS mà không yêu cầu kết nối SMB hoặc NFS.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-50', 'az-204', 50, 'You have an application that uses Azure Blob storage.  
  
You need to update the metadata of the blobs.  
  
Which three methods should you use to develop the solution? Arrange the appropriate methods in the correct order.', '{"statements":[{"id":"slot1","text":"Slot 1:","correctAnswer":"C"},{"id":"slot2","text":"Slot 2:","correctAnswer":"A"},{"id":"slot3","text":"Slot 3:","correctAnswer":"B"}],"choices":[{"key":"A","text":"Metadata.Add"},{"key":"B","text":"SetMetadataAsync"},{"key":"C","text":"FetchAttributesAsync"},{"key":"D","text":"UploadFileStream"},{"key":"E","text":"SetPropertiesAsync"}]}'::jsonb, '["slot1=C","slot2=A","slot3=B"]'::jsonb, 'Để cập nhật siêu dữ liệu blob hiện có, trước tiên hãy truy xuất các thuộc tính và siêu dữ liệu blob, sau đó thêm mục nhập siêu dữ liệu vào bộ sưu tập Siêu dữ liệu của blob và cuối cùng gọi SetMetadataAsync để lưu siêu dữ liệu đã sửa đổi vào Azure Blob Storage. UploadFileStream thay đổi nội dung blob, trong khi SetPropertiesAsync áp dụng thuộc tính blob thay vì bộ sưu tập siêu dữ liệu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-51', 'az-204', 51, 'You need to retain the user agreements.  
  
Where should you store an agreement once it has been completed?', '[{"key":"A","text":"Azure Storage queue"},{"key":"B","text":"Azure Event Hub"},{"key":"C","text":"Azure Service Bus topic"},{"key":"D","text":"Azure Event Grid topic"}]'::jsonb, '["A"]'::jsonb, 'Hàng đợi Bộ lưu trữ Azure lưu giữ các tin nhắn một cách lâu dài cho đến khi người tiêu dùng truy xuất và xử lý chúng, giúp chúng phù hợp để duy trì một thỏa thuận đã hoàn chỉnh cho việc xử lý xuôi dòng không đồng bộ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-52', 'az-204', 52, 'You need to address the capacity issue.  
  
What should you do?', '[{"key":"A","text":"Convert the trigger on the Azure Function to an Azure Blob storage trigger"},{"key":"B","text":"Ensure that the consumption plan is configured correctly to allow scaling"},{"key":"C","text":"Move the Azure Function to a dedicated App Service Plan"},{"key":"D","text":"Update the loop starting on line PC09 to process items in parallel"}]'::jsonb, '["D"]'::jsonb, 'Xử lý song song cho phép xử lý đồng thời các mục độc lập, tăng thông lượng của Chức năng Azure và giảm bớt tắc nghẽn do thực thi vòng lặp tuần tự gây ra. Azure Functions hỗ trợ xử lý đồng thời và tính đồng thời ảnh hưởng trực tiếp đến thông lượng và hành vi quy mô.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you use?', '[{"key":"A","text":"AzCopy"},{"key":"B","text":"Azure Storage Explorer"},{"key":"C","text":"Azure portal"},{"key":"D","text":".NET Storage Client Library"}]'::jsonb, '["A"]'::jsonb, 'AzCopy sao chép dữ liệu giữa các tài khoản lưu trữ Azure và có thể được tích hợp vào các tập lệnh tự động. Nó tạo ra một kế hoạch công việc và các tệp nhật ký cho từng công việc; một công việc bị lỗi hoặc bị hủy có thể được tiếp tục bằng cách sử dụng ID công việc của nó, điều này giúp cho quá trình truyền có thể được phục hồi.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which technology should you use?', '[{"key":"A","text":"Microsoft Graph data connect"},{"key":"B","text":"Microsoft Graph API"},{"key":"C","text":"Microsoft Graph connectors"}]'::jsonb, '["A"]'::jsonb, 'Microsoft Graph Data Connect cung cấp mô hình chấp thuận và kiểm soát chi tiết cho dữ liệu Microsoft 365. Quản trị viên có thể xem xét và phê duyệt các ứng dụng cũng như các tập dữ liệu, cột, phạm vi và đích đến cụ thể được phép truy cập, sao cho phù hợp với kịch bản báo cáo thông tin nhân viên được quản lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which type of test should you configure?', '[{"key":"A","text":"Multi-step web test"},{"key":"B","text":"Standard test"},{"key":"C","text":"URL ping test"},{"key":"D","text":"Custom TrackAvailability test"}]'::jsonb, '["B"]'::jsonb, 'Kiểm tra tính khả dụng tiêu chuẩn hỗ trợ các động từ yêu cầu HTTP bao gồm POST, tiêu đề tùy chỉnh và dữ liệu yêu cầu tùy chỉnh. Nó có thể được cấu hình mà không cần viết và duy trì mã tùy chỉnh cần thiết cho thử nghiệm TrackAvailability tùy chỉnh, giảm thiểu nỗ lực phát triển.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Configure the SearchMode property of the SearchParameters class."},{"key":"B","text":"Configure the QueryType property of the SearchParameters class."},{"key":"C","text":"Configure the Facets property of the SearchParameters class."},{"key":"D","text":"Configure the Filter property of the SearchParameters class."}]'::jsonb, '["B"]'::jsonb, 'Tìm kiếm biểu thức chính quy trong Azure AI Search yêu cầu cú pháp truy vấn Lucene đầy đủ. Đặt thuộc tính `QueryType` để sử dụng trình phân tích cú pháp truy vấn đầy đủ; `SearchMode`, `Facets` và `Filter` không kích hoạt cú pháp biểu thức chính quy.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should the code be completed?', '[]'::jsonb, '[]'::jsonb, 'Trong StackExchange.Redis, `ConnectionMultiplexer.GetDatabase()` trả về `IDatabase`. `IDatabase.KeyDelete` xóa khóa Redis được chỉ định, điều này làm mất hiệu lực dữ liệu `Team` được lưu trong bộ nhớ cache sau khi thay đổi.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-67', 'az-204', 67, 'HOTSPOT –  
  
You need to configure Azure Cosmos DB. Which settings should you use?', '[]'::jsonb, '[]'::jsonb, 'Azure Cosmos DB cung cấp năm mức độ nhất quán; Tính nhất quán mạnh mẽ trả về phiên bản cam kết gần đây nhất của một mục. API SQL, hiện được gọi là API cho NoSQL, là API truy vấn tài liệu gốc của Cosmos DB.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to load the configuration data from Azure App Configuration. How should you complete the code?', '[]'::jsonb, '[]'::jsonb, 'AddAzureAppConfiguration đăng ký nhà cung cấp cấu hình Cấu hình ứng dụng Azure. ManagedIdentityCredential chỉ xác thực thông qua Azure managed identity, đáp ứng yêu cầu sử dụng danh tính ứng dụng web mà không kích hoạt các cơ chế xác thực khác.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which event topic type should you create?', '[{"key":"A","text":"event domain"},{"key":"B","text":"custom"},{"key":"C","text":"system"},{"key":"D","text":"namespace"}]'::jsonb, '["C"]'::jsonb, 'Các nhóm tài nguyên Azure được hỗ trợ các nguồn chủ đề hệ thống Azure Event Grid và các sự kiện thay đổi tài nguyên của chúng được Azure xuất bản. Do đó, chủ đề hệ thống cung cấp nguồn cần thiết mà không cần xây dựng nhà xuất bản sự kiện tùy chỉnh, trong khi đăng ký sự kiện Lưới sự kiện có thể phân phối trực tiếp các sự kiện đến hàng đợi Xe buýt dịch vụ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which ingress type and revision mode should you assign to the container app?', '[{"key":"A","text":"HTTP ingress type and multiple revision mode"},{"key":"B","text":"HTTP ingress type and single revision mode"},{"key":"C","text":"TCP ingress type and multiple revision mode"},{"key":"D","text":"TCP ingress type and single revision mode"}]'::jsonb, '["B"]'::jsonb, 'Mối quan hệ phiên Azure Container Apps (phiên cố định) dựa trên cookie và chỉ được hỗ trợ khi bật tính năng nhập HTTP và ứng dụng sử dụng chế độ sửa đổi duy nhất.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-73', 'az-204', 73, 'You need to retrieve the database connection string.  
  
Which values should you use?', '[]'::jsonb, '[]'::jsonb, 'URI bí mật Azure Key Vault sử dụng tên vault theo sau là `/secrets/` và tên bí mật. `PostgreSQLConn` xác định bí mật chuỗi kết nối cơ sở dữ liệu trong kho `cpandlkeyvault`. Các ứng dụng sử dụng tham chiếu Key Vault làm giá trị cài đặt môi trường/ứng dụng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Configure and apply header-based versioning."},{"key":"B","text":"Create and publish a product."},{"key":"C","text":"Configure and apply a custom policy."},{"key":"D","text":"Add a new revision to the API."},{"key":"E","text":"Configure and apply query string-based versioning."}]'::jsonb, '["D"]'::jsonb, 'Các bản sửa đổi Quản lý API Azure hỗ trợ các thay đổi được kiểm soát, không vi phạm mà không làm gián đoạn người gọi bản sửa đổi hiện tại. Một bản sửa đổi có thể được chỉnh sửa và kiểm tra trước khi nó trở thành hiện hành, được xuất bản với mục nhập nhật ký thay đổi dành cho nhà phát triển và được khôi phục bằng cách tạo bản sửa đổi trước đó thành hiện hành. [Revisions in Azure API Management](https://learn.microsoft.com/en-us/azure/api-management/api-management-revisions)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure Azure Cosmos DB to query the data. Which values should you use?', '[]'::jsonb, '[]'::jsonb, 'Azure Cosmos DB dành cho NoSQL, trước đây gọi là Core (SQL), lưu trữ tài liệu JSON và cung cấp hỗ trợ truy vấn SQL. Khóa phân vùng phải có nhiều giá trị và phân phối dữ liệu cũng như đơn vị yêu cầu một cách đồng đều; `item id` cung cấp lượng số cao hơn nhiều so với tên hoặc số lượng gói.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which features should you use?', '[]'::jsonb, '[]'::jsonb, 'Azure Container Apps Nhật ký phát trực tuyến nhật ký bảng điều khiển vùng chứa gần thời gian thực. Bảng điều khiển vùng chứa kết nối với bảng điều khiển Linux bên trong vùng chứa, cho phép khắc phục sự cố và gỡ lỗi trong vùng chứa.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Create four partitions. Update serviceA to use one partition and serviceB to use three partitions."},{"key":"B","text":"Enable and configure Azure Event Hubs Capture."},{"key":"C","text":"Create an Azure Event Hubs dedicated cluster. Configure the capacity units to one and the scaling units to two."},{"key":"D","text":"Create and configure an Azure Schema Registry in Event Hubs. Update serviceA and serviceB to validate message schemas."},{"key":"E","text":"Create four consumer groups. Update serviceA to use one consumer group and serviceB to use three consumer groups."}]'::jsonb, '["A"]'::jsonb, 'Các phân vùng cung cấp đường dẫn xuất bản song song trong trung tâm sự kiện. Việc phân bổ một phân vùng cho nhà xuất bản 1.000 sự kiện mỗi giây và ba phân vùng cho nhà xuất bản 3.000 sự kiện mỗi giây sẽ cung cấp bốn đường dẫn xuất bản song song đồng thời tránh được chi phí bổ sung và chức năng không liên quan của cụm chuyên dụng, Capture, Schema Register hoặc nhóm người tiêu dùng. Ở cấp Tiêu chuẩn, không gian tên cũng phải có đủ đơn vị thông lượng vì đơn vị thông lượng kiểm soát tổng công suất đầu vào; mỗi cái cung cấp tới 1.000 sự kiện xâm nhập mỗi giây. [Microsoft Learn: Scaling with Event Hubs](https://learn.microsoft.com/en-us/azure/event-hubs/event-hubs-scalability)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which command should you use?', '[{"key":"A","text":"az ad signed-in-user list-owned-objects"},{"key":"B","text":"az ad sp credential list"},{"key":"C","text":"az ad user show"},{"key":"D","text":"Get-AzResource"}]'::jsonb, '["D"]'::jsonb, 'Đối với máy ảo có system-assigned managed identity, ID đối tượng của managed identity là nhận dạng tài nguyên `PrincipalId`. `Get-AzResource` truy xuất tài nguyên VM và hiển thị giá trị này thông qua `Identity.PrincipalId`.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"metadata"},{"key":"B","text":"type"},{"key":"C","text":"authenticationRef"},{"key":"D","text":"name"},{"key":"E","text":"metricType"}]'::jsonb, '["A","B"]'::jsonb, 'Quy tắc chia tỷ lệ tùy chỉnh Azure Container Apps sử dụng bộ chia tỷ lệ KEDA `type` để xác định bộ chia tỷ lệ của Trung tâm sự kiện và `metadata` để cung cấp các cài đặt và ngưỡng chia tỷ lệ cần thiết cho Trung tâm sự kiện của bộ chia tỷ lệ. Tài liệu mở rộng Ứng dụng vùng chứa của Microsoft ánh xạ các trường kích hoạt KEDA này tới các thuộc tính `type` và `metadata` của quy tắc tùy chỉnh.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you implement?', '[{"key":"A","text":"HTTP readiness probe"},{"key":"B","text":"TCP readiness probe"},{"key":"C","text":"HTTP startup probe"},{"key":"D","text":"TCP liveness probe"},{"key":"E","text":"HTTP liveness probe"}]'::jsonb, '["A"]'::jsonb, 'HTTP readiness probe kiểm tra xem bản sao có sẵn sàng nhận yêu cầu đến thông qua điểm cuối HTTP hay không. Azure Container Apps sử dụng trạng thái sẵn sàng để xác định xem bản sửa đổi có thể nhận được lưu lượng truy cập hay không, trong khi các thăm dò khởi động và hoạt động phục vụ các mục đích kiểm tra tình trạng khác nhau.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which alert type should you use?', '[{"key":"A","text":"Metric alert using multiple time series"},{"key":"B","text":"Metric alert using dynamic thresholds"},{"key":"C","text":"Log alert using multiple time series"},{"key":"D","text":"Log alert using dynamic thresholds"}]'::jsonb, '["B"]'::jsonb, 'Cảnh báo số liệu có ngưỡng động phù hợp với số liệu bán hàng có hành vi bình thường tuân theo mẫu cuối tuần. Ngưỡng động tìm hiểu hành vi của số liệu lịch sử, bao gồm các mẫu hàng giờ, hàng ngày và hàng tuần, đồng thời xác định các sai lệch bất thường, chẳng hạn như doanh số bán hàng giảm bất ngờ có thể cho thấy lỗi kỹ thuật. Chuỗi nhiều thời gian nhằm mục đích giám sát riêng chuỗi nhiều thứ nguyên hoặc tài nguyên, điều này không bắt buộc ở đây.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to implement the Azure Event Hub. Which settings should you use?', '[]'::jsonb, '[]'::jsonb, 'Khóa phân vùng phải là Đường cao tốc để tất cả các sự kiện cho một đường cao tốc nhất định được định tuyến nhất quán đến cùng một phân vùng, duy trì thứ tự trên mỗi đường cao tốc mà việc phân tích chuỗi thời gian yêu cầu. Với Đường cao tốc làm khóa, chỉ có sáu giá trị khóa riêng biệt, do đó, sáu phân vùng là số lượng chính xác: nó cung cấp một kênh chuyên dụng, có thể đọc độc lập trên mỗi đường cao tốc, tối đa hóa thông lượng song song và giảm thiểu độ trễ. Việc chọn 12 phân vùng là sai vì sáu khóa riêng biệt không bao giờ có thể chứa nhiều hơn sáu phân vùng, khiến sáu phân vùng bổ sung bị trống vĩnh viễn; số lượng phòng ban/phiên bản (4 x 3 = 12) thúc đẩy tính song song phía người tiêu dùng, chứ không phải số lượng phân vùng, vì mỗi phòng ban là nhóm người tiêu dùng riêng và có thể đọc tất cả sáu phân vùng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you implement?', '{"statements":[{"id":"event_source","text":"Event Source","correctAnswer":"D"},{"id":"event_receiver","text":"Event Receiver","correctAnswer":"B"},{"id":"event_handler","text":"Event Handler","correctAnswer":"F"}],"choices":[{"key":"A","text":"Azure Event Hub"},{"key":"B","text":"Azure Event Grid"},{"key":"C","text":"Azure Service Bus"},{"key":"D","text":"Azure Blob Storage"},{"key":"E","text":"Azure App Service"},{"key":"F","text":"Azure Logic App"}]}'::jsonb, '["event_source=D","event_receiver=B","event_handler=F"]'::jsonb, 'Azure Blob Storage tạo ra các sự kiện do blob tạo. Azure Event Grid định tuyến và lọc các sự kiện này với số lần thử lại trong 24 giờ và phân phối chờ theo cấp số nhân. Ứng dụng Azure Logic là trình xử lý Lưới sự kiện được hỗ trợ và có thể xử lý sự kiện trước khi ghi vào Azure Cosmos DB.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which values should you use for the configuration settings?', '[]'::jsonb, '[]'::jsonb, 'Bộ đệm tích hợp Azure Cosmos DB chạy trên cổng chuyên dụng và yêu cầu kết nối ở chế độ cổng bằng cổng chuyên dụng. Nó hỗ trợ tính nhất quán của phiên và cuối cùng; tính nhất quán mạnh mẽ đọc bỏ qua bộ đệm. Do đó, phiên là mức nhất quán được hỗ trợ cao nhất.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Update the app so that it can use the feature flag. Which values should you select?', '[]'::jsonb, '[]'::jsonb, '`FeatureGate` là thuộc tính quản lý tính năng ASP.NET Core giúp điều khiển bộ điều khiển MVC hoặc hành động bằng cờ tính năng được đặt tên. `AddAzureAppConfiguration` thêm Cấu hình ứng dụng Azure vào quy trình cấu hình ứng dụng. Điểm cuối Cấu hình ứng dụng Azure sử dụng mẫu máy chủ có tên cửa hàng `https://<store-name>.azconfig.io`, biến `https://appfeatureflagstore.azconfig.io` thành điểm cuối phù hợp.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783807939264-fncriksl.png', 'published'),
...
You need to implement code that acquires a token by using the certificate. How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, '`ConfidentialClientApplicationBuilder` xây dựng ứng dụng khách bí mật MSAL được định cấu hình bằng chứng chỉ. `AcquireTokenForClient` chấp nhận bộ sưu tập phạm vi được yêu cầu; đối với thông tin xác thực của khách hàng, Microsoft Graph sử dụng phạm vi tài nguyên `.default`.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure the service. How should you complete the script?', '[]'::jsonb, '[]'::jsonb, 'Chia tỷ lệ cho mỗi ứng dụng phải được bật trên gói Dịch vụ ứng dụng bằng cách đặt `PerSiteScaling` thành true. Số lượng phiên bản trên mỗi ứng dụng được kiểm soát bởi `SiteConfig.NumberOfWorkers`, do đó, việc đặt thành 1 sẽ giới hạn ExcelParser ở một phiên bản riêng biệt mà không giới hạn các ứng dụng web khác trong gói dùng chung.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which method should you use in your code?', '[{"key":"A","text":"GetProperties method of the QueueClient class"},{"key":"B","text":"GetProperties method of the QueueServiceClient class"},{"key":"C","text":"PeekMessages method of the QueueClient class"},{"key":"D","text":"GetStatistics method of the QueueServiceClient class"}]'::jsonb, '["A"]'::jsonb, '`QueueClient.GetProperties` truy xuất các thuộc tính của một hàng đợi cụ thể và giá trị `QueueProperties.ApproximateMessagesCount` được trả về cung cấp số lượng tin nhắn gần đúng của hàng đợi. Đây là hoạt động SDK trực tiếp để lấy ước tính mà không cần truy xuất thông báo.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-104', 'az-204', 104, 'Solution: You configure Basic gateway credentials for the HTTP(S) endpoint.  
  
Does this solution satisfy the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Quản lý API Azure có thể xác thực với API HTTP(S) phụ trợ bằng cách sử dụng thông tin xác thực Cơ bản. Tên người dùng và mật khẩu đã định cấu hình sẽ được gửi đến phần phụ trợ trong tiêu đề Ủy quyền HTTP, tiêu đề này cung cấp xác thực phụ trợ cho dịch vụ API.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-105', 'az-204', 105, 'Solution: Configure Client cert gateway credentials for the HTTP(S) endpoint.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Quản lý API Azure hỗ trợ xác thực chứng chỉ ứng dụng khách từ cổng đến dịch vụ HTTP(S) phụ trợ. Thông tin xác thực cổng chứng chỉ ứng dụng khách định cấu hình chứng chỉ mà Quản lý API cung cấp cho phần phụ trợ, đáp ứng yêu cầu xác thực phụ trợ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-106', 'az-204', 106, 'Solution: Configure **Client cert** gateway credentials for the Azure resource.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Quản lý API Azure hỗ trợ xác thực TLS lẫn nhau cho dịch vụ phụ trợ bằng cách đặt cấu hình chứng chỉ ứng dụng khách trong thông tin xác thực Cổng của API. Azure App Service có thể được định cấu hình để xác thực chứng chỉ ứng dụng khách, do đó, điều này cung cấp xác thực phụ trợ cho API RESTful.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-107', 'az-204', 107, 'Solution: You configure Basic gateway credentials for the Azure resource.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Thông tin xác thực cổng cơ bản gửi tên người dùng và mật khẩu đến chương trình phụ trợ bằng xác thực HTTP Basic; họ không định cấu hình xác thực phụ trợ Azure App Service cho tài nguyên Azure. Quản lý API hỗ trợ định cấu hình chứng chỉ ứng dụng khách làm xác thực cổng cho dịch vụ phụ trợ, đây là cơ chế xác thực phụ trợ có thể áp dụng tại đây.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which configuration values should you use?', '[]'::jsonb, '[]'::jsonb, 'Bộ nhớ đệm Azure dành cho Redis sử dụng cài đặt chẩn đoán Azure Monitor để thu thập nhật ký kết nối máy khách. Việc gửi các nhật ký đó đến không gian làm việc Log Analytics giúp chúng có thể truy vấn được để phân tích bảo mật; bảng ACRConnectedClientList bao gồm các trường hoạt động Redis và vùng truy cập. Đối với tài nguyên khu vực, Trung tâm sự kiện và đích lưu trữ phải nằm trong khu vực của tài nguyên được giám sát, trong khi không gian làm việc Log Analytics hỗ trợ vị trí phân tích Trung tâm Hoa Kỳ được yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Configure the website to use Azure AD B2C."},{"key":"B","text":"In Azure AD, create a new conditional access policy."},{"key":"C","text":"Upgrade to Azure AD Premium."},{"key":"D","text":"In Azure AD, enable application proxy."},{"key":"E","text":"In Azure AD conditional access, enable the baseline policy."}]'::jsonb, '["B","C"]'::jsonb, 'Truy cập có điều kiện của Microsoft Entra có thể yêu cầu xác thực đa yếu tố khi người dùng truy cập vào một ứng dụng đã chọn. Truy cập có điều kiện yêu cầu cấp phép Microsoft Entra ID P1 hoặc P2 (trước đây là Azure AD Premium), do đó, các hành động bắt buộc là nâng cấp lên Azure AD Premium và tạo chính sách Truy cập có điều kiện yêu cầu MFA.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which SLA should you use?', '[{"key":"A","text":"at least two days"},{"key":"B","text":"between one and 15 hours"},{"key":"C","text":"at least one day"},{"key":"D","text":"between zero and 60 minutes"}]'::jsonb, '["B"]'::jsonb, 'Các đốm màu trong archive tier đang ngoại tuyến và phải được cấp nước lại ở cấp độ trực tuyến trước khi có thể truy cập được. Quá trình bù nước cho kho lưu trữ có mức độ ưu tiên tiêu chuẩn có thể mất tới 15 giờ, do đó SLA phục hồi hiện hành là từ 1 đến 15 giờ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which configuration should you use?', '[]'::jsonb, '[]'::jsonb, 'HTTP và webhooks cung cấp trình kích hoạt HTTP cho Hàm Azure. Liên kết đầu vào Blob Storage đọc dữ liệu blob vào hàm và liên kết đầu ra Azure Cosmos DB ghi tài liệu đã tạo vào Cosmos DB.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the Azure Resource Manager template?', '[]'::jsonb, '[]'::jsonb, 'Ngưỡng động tìm hiểu mẫu lỗi thông thường, bao gồm lỗi ngẫu nhiên của bên thứ ba và mẫu lưu lượng truy cập hàng tháng, đồng thời cảnh báo về những sai lệch bất thường. `Http5xx` đo lỗi máy chủ; `Http4xx` đo lỗi yêu cầu của khách hàng. Độ nhạy thấp sử dụng ngưỡng động lỏng lẻo hơn và do đó tạo ra ít cảnh báo hơn.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"ID token signature"},{"key":"B","text":"ID token claims"},{"key":"C","text":"HTTP response code"},{"key":"D","text":"Azure AD endpoint URI"},{"key":"E","text":"Azure AD tenant ID"}]'::jsonb, '["A","B"]'::jsonb, 'Mã thông báo ID chỉ thiết lập danh tính của người dùng sau khi chữ ký của nó được xác thực và các xác nhận quyền sở hữu của nó đã được xác minh. Xác thực chữ ký xác nhận mã thông báo được phát hành bởi nhà cung cấp danh tính đáng tin cậy, trong khi xác thực yêu cầu xác nhận các thuộc tính như đối tượng và nhà phát hành dự kiến. Microsoft Entra ID ghi lại rằng các ứng dụng xác thực mã thông báo ID phải xác thực cả chữ ký mã thông báo và các xác nhận quyền sở hữu có thể áp dụng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
**NOTE:** Each correct selection is worth one point.', '[{"key":"A","text":"adding data to documents"},{"key":"B","text":"deleting documents"},{"key":"C","text":"creating documents"},{"key":"D","text":"overwriting existing documents"}]'::jsonb, '["A","B"]'::jsonb, 'Với tính bất biến ở cấp độ phiên bản, việc ghi đè blob sẽ tạo ra một phiên bản mới, do đó, việc lưu giữ pháp lý của phiên bản hiện tại không chặn việc ghi đè. Các đốm màu mới cũng có thể được tạo ra. `AllowProtectedAppendWrites` cho phép thêm vào khi chính sách lưu giữ theo thời gian đang hoạt động; sau khi chính sách đó hết hạn, lệnh lưu giữ pháp lý có hiệu lực sẽ ngăn việc sửa đổi tài liệu được lưu giữ, bao gồm cả việc thêm dữ liệu. Việc lưu giữ hợp pháp cũng ngăn chặn việc xóa cho đến khi nó được xóa rõ ràng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Identify the configuration options that meet the requirements.', '[]'::jsonb, '[]'::jsonb, 'Luôn bật giúp máy chủ Chức năng luôn sẵn sàng khi một ứng dụng chạy trên gói Dịch vụ ứng dụng chuyên dụng, điều này cần thiết để chức năng được kích hoạt hàng đợi tiếp tục xử lý tin nhắn thay vì ở trạng thái rảnh. Cấp Cơ bản là cấp được liệt kê ít tốn kém nhất hỗ trợ Luôn bật; Miễn phí và chia sẻ không hỗ trợ nó. Managed identity và việc triển khai liên tục không giải quyết được tính khả dụng của máy chủ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure the website''s Azure AD application so that users'' permissions can be used with the Azure Blob containers. Each setting may be used once, more than once, or not at all.', '{"statements":[{"id":"azure_storage_permission","text":"Azure Storage — Permission","correctAnswer":"E"},{"id":"azure_storage_type","text":"Azure Storage — Type","correctAnswer":"C"},{"id":"microsoft_graph_type","text":"Microsoft Graph — Type","correctAnswer":"C"}],"choices":[{"key":"A","text":"client_id"},{"key":"B","text":"profile"},{"key":"C","text":"delegated"},{"key":"D","text":"application"},{"key":"E","text":"user_impersonation"}]}'::jsonb, '["azure_storage_permission=E","azure_storage_type=C","microsoft_graph_type=C"]'::jsonb, 'Azure Storage hiển thị `user_impersonation` dưới dạng phạm vi được ủy quyền duy nhất. Nó cho phép ứng dụng hoạt động trong phạm vi quyền của người dùng đã đăng nhập, do đó Azure Storage RBAC xác định quyền truy cập hiệu quả của người dùng vào các đốm màu. Quyền hiện có của Microsoft Graph `User.Read` cũng được ủy quyền khi ứng dụng hoạt động thay mặt cho người dùng đã đăng nhập.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-122', 'az-204', 122, 'You need to ensure that validation testing is initiated according to the requirements.  
  
How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, 'Cơ quan đăng ký vùng chứa Azure xuất bản `Microsoft.ContainerRegistry.ImagePushed` khi hình ảnh được đẩy. Dữ liệu sự kiện Lưới sự kiện của nó bao gồm `data.target.repository`, trong khi sự kiện Lưới sự kiện `topic` xác định tài nguyên đăng ký. Các hoạt động kiểm tra này giới hạn việc kiểm tra xác thực đối với các hình ảnh được đẩy trong kho lưu trữ `contentanalysisservice` của sổ đăng ký `contosoimages`.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Review the API connections."},{"key":"B","text":"Review the activity log."},{"key":"C","text":"Review the run history."},{"key":"D","text":"Review the trigger history."}]'::jsonb, '["C","D"]'::jsonb, 'Lịch sử kích hoạt Power Automate cho biết liệu một sự kiện đã được nhận và trình kích hoạt có được kích hoạt hay không, trong khi lịch sử chạy hiển thị trạng thái và lỗi chi tiết của từng lần thực thi quy trình làm việc. Cùng nhau, chúng phân biệt sự cố kích hoạt với lỗi trong hành động sau này của quy trình làm việc.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which feature should you implement?', '[]'::jsonb, '[]'::jsonb, 'Hàng đợi thư chết ghi lại các tài liệu không thể xử lý được, cho phép quá trình gọi lại quá trình xử lý hoàn tất để bộ xử lý nguồn cấp dữ liệu thay đổi có thể kiểm tra điểm và tiếp tục đến các đợt sau. Thông báo về vòng đời cung cấp các trình xử lý như `WithErrorNotification` cho các lỗi của bộ xử lý và ủy quyền. Một container cho thuê duy trì trạng thái điểm kiểm tra và phối hợp; công cụ ước tính nguồn cấp dữ liệu thay đổi chỉ đo lường công việc chưa hoàn thành.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-128', 'az-204', 128, 'You must resolve an issue with notification latency.  
  
Which two actions should you take? Each correct answer represents part of the solution.  
  
> NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Set Always On to true."},{"key":"B","text":"Ensure that the Azure Function is using an App Service plan."},{"key":"C","text":"Set Always On to false."},{"key":"D","text":"Ensure that the Azure Function is set to use a consumption plan."}]'::jsonb, '["A","B"]'::jsonb, 'Trên gói Dịch vụ ứng dụng chuyên dụng, thời gian chạy Azure Functions có thể không hoạt động sau một thời gian không hoạt động. Việc bật **Luôn bật** giúp thiết bị luôn chạy và tránh độ trễ khởi động liên quan. Cài đặt Luôn bật chỉ khả dụng khi chức năng này sử dụng gói Dịch vụ ứng dụng; Các ứng dụng kế hoạch tiêu dùng được nền tảng kích hoạt tự động.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which of the following should you implement?', '[{"key":"A","text":"Azure AD token."},{"key":"B","text":"Azure Multi-Factor authentication."},{"key":"C","text":"Active Directory integrated authentication."},{"key":"D","text":"OATH software tokens."}]'::jsonb, '["C"]'::jsonb, 'Xác thực tích hợp Active Directory sử dụng Xác thực Windows tích hợp với thông tin xác thực Active Directory tại chỗ hiện có của nhà phát triển, miễn là thư mục tại chỗ được nối với Microsoft Entra ID. Trên máy tham gia miền, nó cho phép truy cập vào Azure SQL mà không cần nhắc thông tin xác thực, giảm thiểu lời nhắc xác thực.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure the App Service plan for the Web App. How should you configure it?', '[]'::jsonb, '[]'::jsonb, 'Bốn khách hàng yêu cầu bốn phiên bản WebJob đơn lẻ chuyên dụng khi mỗi phiên bản xử lý các bản cập nhật của một khách hàng. Môi trường dịch vụ ứng dụng cung cấp khả năng triển khai mạng biệt lập cần thiết và sử dụng bậc giá Dịch vụ ứng dụng biệt lập. Các vị trí triển khai được hỗ trợ ở bậc Biệt lập và bốn là số lượng phiên bản được cung cấp thấp nhất đáp ứng yêu cầu của khách hàng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to obtain a claim that uniquely identifies a user. Which claim type should you use?', '[{"key":"A","text":"aud"},{"key":"B","text":"nonce"},{"key":"C","text":"oid"},{"key":"D","text":"idp"}]'::jsonb, '["C"]'::jsonb, 'Xác nhận quyền sở hữu `oid` là mã định danh đối tượng bất biến cho tài khoản người dùng. Nó nhận dạng duy nhất người dùng trên các ứng dụng sử dụng nền tảng nhận dạng của Microsoft.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution meet the goal?', '[{"key":"A","text":"No"},{"key":"B","text":"Yes"}]'::jsonb, '["B"]'::jsonb, 'Azure App Service hỗ trợ khởi động trước khi hoán đổi tùy chỉnh thông qua phần tử `applicationInitialization` trong `Web.config`. Nền tảng chạy các hành động khởi tạo được chỉ định và đợi quá trình khởi động tùy chỉnh kết thúc trước khi hoàn tất việc hoán đổi vị trí.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You are preparing to configure the service that processes a queue data item. Which service should you use?', '[{"key":"A","text":"Logic Apps"},{"key":"B","text":"WebJobs"},{"key":"C","text":"Flow"},{"key":"D","text":"Functions"}]'::jsonb, '["B"]'::jsonb, 'Azure WebJobs lưu trữ xử lý nền cho ứng dụng Dịch vụ ứng dụng. SDK WebJobs cung cấp trình kích hoạt hàng đợi tự động gọi một hàm bất cứ khi nào nhận được thông báo hàng đợi mới.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Each code segment can be used once, more than once, or not at all.', '{"statements":[{"id":"client","text":"Slot 1:","correctAnswer":"D"},{"id":"credential","text":"Slot 2:","correctAnswer":"A"}],"choices":[{"key":"A","text":"DefaultAzureCredential"},{"key":"B","text":"ClientSecretCredential"},{"key":"C","text":"CloudClients"},{"key":"D","text":"SecretClient"}]}'::jsonb, '["client=D","credential=A"]'::jsonb, '`SecretClient` là ứng dụng khách SDK bí mật Azure Key Vault. Hàm tạo của nó lấy URI vault và `TokenCredential`; `DefaultAzureCredential` cung cấp thông tin xác thực đó và chọn một bản phát triển có sẵn hoặc danh tính được lưu trữ trên Azure khi chạy.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which policy should you use?', '[{"key":"A","text":"ip-filter"},{"key":"B","text":"quota-by-key"},{"key":"C","text":"rate-limit-by-key"},{"key":"D","text":"rate-limit"}]'::jsonb, '["C"]'::jsonb, 'Chính sách `rate-limit-by-key` giới hạn tốc độ cuộc gọi cho mỗi khóa bộ đếm trong khoảng thời gian gia hạn được chỉ định. Việc đặt khóa bộ đếm cho địa chỉ IP của người gọi sẽ áp dụng giới hạn mỗi phút một cách độc lập cho từng địa chỉ IP, giúp bảo vệ cơ sở dữ liệu phụ trợ với ràng buộc yêu cầu mỗi phút.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which token type should be used?', '[]'::jsonb, '[]'::jsonb, 'Mã thông báo ID xác thực và nhận dạng người dùng thông qua xác nhận danh tính. Mã thông báo truy cập mang thông tin ủy quyền cho các API được bảo vệ. Mã thông báo làm mới cho phép khách hàng nhận mã thông báo truy cập thay thế mà không yêu cầu người dùng đăng nhập lại. Xác nhận SAML là mã thông báo dựa trên XML được WS-Federation hỗ trợ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Identify the app hosting plan and the maximum time the app function can take to respond to incoming requests.', '[]'::jsonb, '[]'::jsonb, 'Gói Premium hỗ trợ mở rộng quy mô theo sự kiện và chạy Azure Functions trong các bộ chứa Linux tùy chỉnh. Hàm được kích hoạt HTTP phải trả về phản hồi HTTP trong vòng 230 giây do hết thời gian chờ của Azure Load Balancer; giới hạn phản hồi này áp dụng ngay cả khi thời gian chờ thực thi hàm không bị giới hạn.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-172', 'az-204', 172, 'You need to secure the Shipping Logic App.  
  
What should you use?', '[{"key":"A","text":"Azure App Service Environment (ASE)"},{"key":"B","text":"Integration Service Environment (ISE)"},{"key":"C","text":"VNet service endpoint"},{"key":"D","text":"Azure AD B2B integration"}]'::jsonb, '["B"]'::jsonb, 'Môi trường dịch vụ tích hợp (ISE) cung cấp thời gian chạy Ứng dụng Azure Logic chuyên dụng có thể tích hợp trực tiếp với các hệ thống trong mạng ảo, bao gồm các hệ thống tại chỗ thông qua ExpressRoute, cung cấp khả năng cách ly và bảo mật mạng cần thiết.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which HTTP header should be included on the allow list?', '[{"key":"A","text":"Authorization"},{"key":"B","text":"WebHook-Request-Callback"},{"key":"C","text":"Resource"},{"key":"D","text":"WebHook-Request-Origin"}]'::jsonb, '["D"]'::jsonb, 'Azure Web PubSub xác thực từng webhook ngược dòng được định cấu hình bằng cách sử dụng tính năng chống lạm dụng CloudEvents. Yêu cầu xác thực chứa tiêu đề `WebHook-Request-Origin` và webhook phải nhận dạng tiêu đề đó và phản hồi bằng giá trị `WebHook-Allowed-Origin` thích hợp. Do đó, CDN phía trước Chức năng Azure phải cho phép `WebHook-Request-Origin` đi qua.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which file and function names should you use?', '[]'::jsonb, '[]'::jsonb, 'Tính đồng thời động được bật trong Host.json và được quản lý độc lập cho từng chức năng được hỗ trợ. Các loại trình kích hoạt được hỗ trợ bao gồm Azure Queue Storage, trong khi trình kích hoạt HTTP và Bộ hẹn giờ không sử dụng đồng thời động; do đó f3 là hàm có thể áp dụng.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783879214981-zx8zr6vw.png', 'published'),
...
For each of the following statements, select **Yes** if the statement is true. Otherwise, select **No**.', '{"statements":[{"id":"1","text":"The code creates an infinite lease","correctAnswer":"Yes"},{"id":"2","text":"The code at line 06 always creates a new blob","correctAnswer":"No"},{"id":"3","text":"The finally block releases the lease","correctAnswer":"Yes"}],"choices":[{"key":"Yes","text":"Yes"},{"key":"No","text":"No"}]}'::jsonb, '["1:Yes","2:No","3:Yes"]'::jsonb, 'Thời hạn thuê vô hiệu sẽ tạo ra một hợp đồng thuê vô hạn. Việc lấy tham chiếu block-blob không tạo ra blob. Khối cuối cùng phá vỡ bất kỳ hợp đồng thuê nguồn đang hoạt động nào với thời gian nghỉ bằng 0, khiến nó có sẵn ngay lập tức.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783790236271-3ehuu9u7.jpg', 'published'),
  ('az204-180', 'az-204', 180, 'Solution: Scale up ASPPlan1 to the Premium V2 pricing tier.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Tự động chia tỷ lệ theo lịch trình cho Azure App Service được hỗ trợ ở cấp Tiêu chuẩn trở lên. Do đó, Premium V2 không phải là mức chi phí thấp nhất có thể cung cấp chức năng cần thiết, do đó, việc mở rộng sang Premium V2 không giúp giảm thiểu chi phí.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-181', 'az-204', 181, 'Solution: Scale down ASPPlan1 to the Shared pricing tier.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Cấp Dịch vụ ứng dụng dùng chung sử dụng điện toán dùng chung và không thể mở rộng quy mô. Tính năng tự động chia tỷ lệ theo lịch trình áp dụng cho gói Dịch vụ ứng dụng và yêu cầu khả năng Tự động chia tỷ lệ có sẵn ở các bậc Tiêu chuẩn trở lên; do đó, gói Chia sẻ cấp độ không thể đáp ứng yêu cầu tự động điều chỉnh quy mô theo lịch trình, mặc dù chi phí thấp hơn. [Azure App Service Plans](https://learn.microsoft.com/en-us/azure/app-service/overview-hosting-plans)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-182', 'az-204', 182, 'Solution: Scale out APSPlan1.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Azure Autoscale dựa trên lịch trình cho gói Dịch vụ ứng dụng chỉ được hỗ trợ ở cấp Tiêu chuẩn trở lên; gói B1 cơ bản không thể sử dụng nó. Việc mở rộng quy mô cũng bổ sung thêm các phiên bản VM, mỗi phiên bản đều phát sinh chi phí thay vì giảm thiểu chi phí chạy ứng dụng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Select the hosting model and the schedule-expression assignment method.', '[]'::jsonb, '[]'::jsonb, 'Trình kích hoạt hẹn giờ chỉ có thể sử dụng lịch trình TimeSpan trên gói Dịch vụ ứng dụng. Lịch trình liên kết kích hoạt được xác định trong function.json; Host.json dành cho cấu hình trên toàn máy chủ, trong khi cài đặt ứng dụng chỉ có thể được tham chiếu dưới dạng giá trị lịch trình.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to prevent this exception. What should you do?', '[{"key":"A","text":"In the host.json file, decrease the value of the batchSize option"},{"key":"B","text":"Convert the trigger to Azure Event Hub"},{"key":"C","text":"Convert the Azure Function to the Premium plan"},{"key":"D","text":"In the function.json file, change the value of the type option to queueScaling"}]'::jsonb, '["A"]'::jsonb, 'Trình kích hoạt hàng đợi Lưu trữ Azure truy xuất và xử lý các thông báo hàng đợi đồng thời. Cài đặt `batchSize` trong `host.json` kiểm soát số lượng tin nhắn được xử lý song song, do đó, việc giảm cài đặt này sẽ làm giảm các lệnh gọi Hàm Azure đồng thời và mức sử dụng kết nối Azure SQL Database đồng thời của chúng. Điều này tránh làm cạn kiệt nhóm kết nối SQL.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the CLI command?', '[]'::jsonb, '[]'::jsonb, '`az containerapp create` triển khai Ứng dụng Container. Quy tắc thang đo HTTP đánh giá các yêu cầu trong khoảng thời gian 15 giây, do đó, 300 yêu cầu trong 15 giây tương ứng với mục tiêu được định cấu hình là 20 yêu cầu mỗi giây. Với tính năng xâm nhập HTTP và quy tắc tỷ lệ HTTP, Azure Container Apps có thể kích hoạt từ 0 bản sao khi có yêu cầu đến. Do đó, `--min-replicas 0` giảm thiểu chi phí nhàn rỗi trong khi vẫn duy trì khả năng mở rộng theo yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do first?', '[{"key":"A","text":"Enable autoscaling on the Web App."},{"key":"B","text":"Switch to the Premium App Service tier plan."},{"key":"C","text":"Switch to the Standard App Service tier plan."},{"key":"D","text":"Switch to the Azure App Services consumption plan."}]'::jsonb, '["C"]'::jsonb, 'Cấp chia sẻ (D1) không thể mở rộng quy mô. Tính năng tự động thay đổi quy mô có sẵn ở cấp Tiêu chuẩn trở lên, do đó, việc chuyển gói Dịch vụ ứng dụng sang Tiêu chuẩn sẽ cung cấp khả năng tự động thay đổi quy mô dựa trên CPU cần thiết với chi phí thấp hơn so với Premium.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which value should you use?', '[{"key":"A","text":"$$_.Type -eq \"ping\""},{"key":"B","text":"$$_.WebTestKind -eq \"ping\""},{"key":"C","text":"$$_.WebTestKind -eq \"standard\""},{"key":"D","text":"$$_.Type -eq \"standard\""}]'::jsonb, '["B"]'::jsonb, 'Kiểm tra tính khả dụng của ping URL cổ điển có `WebTestKind` được đặt thành `ping`. `Get-AzApplicationInsightsWebTest` hiển thị thuộc tính này để lọc, trong khi `Type` đại diện cho loại tài nguyên Azure; `standard` biểu thị bài kiểm tra Tiêu chuẩn chứ không phải bài kiểm tra ping Cổ điển.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Use a unique consumer group for each road."},{"key":"B","text":"Ensure each road stores events in a different partition."},{"key":"C","text":"Ensure each road has a unique connection string."},{"key":"D","text":"Use a unique application group for each road."}]'::jsonb, '["D"]'::jsonb, 'Các nhóm ứng dụng Azure Event Hubs hỗ trợ các chính sách điều chỉnh và có sẵn ở cấp Premium. Việc chỉ định mỗi con đường cho nhóm ứng dụng riêng của nó sẽ cho phép có chính sách điều tiết riêng biệt cho con đường đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the configuration?', '[]'::jsonb, '[]'::jsonb, 'Chính sách `mock-response` trả về phản hồi API mô phỏng và hủy quá trình thực thi quy trình thông thường. Việc áp dụng nó trong phần gửi đến sẽ ngăn APIM chuyển tiếp yêu cầu đến phần phụ trợ chưa hoàn thành. Mã trạng thái 200 trả về phản hồi kiểm tra thành công.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the code?', '[]'::jsonb, '[]'::jsonb, '`IDurableEntityClient` là giao diện máy khách Chức năng bền vững cho các hoạt động của thực thể. `SignalEntityAsync` gửi tín hiệu không đồng bộ một chiều và không đợi thực thể trả về kết quả, điều này giúp giảm thiểu độ trễ yêu cầu để ghi lại phiếu bầu. `CallEntityAsync` chờ phản hồi và không phù hợp với thao tác chỉ cập nhật này.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which three actions should you perform in sequence?', '{"statements":[{"id":"slot1","text":"1.","correctAnswer":"B"},{"id":"slot2","text":"2.","correctAnswer":"A"},{"id":"slot3","text":"3.","correctAnswer":"E"}],"choices":[{"key":"A","text":"Modify the client certificate mode of the Configuration settings of App1."},{"key":"B","text":"Upgrade the pricing tier of the App Service plan."},{"key":"C","text":"Add an identity provider to the Authentication settings of App1."},{"key":"D","text":"Downgrade the pricing tier of the App Service plan."},{"key":"E","text":"Modify the certificate exclusion path of the Configuration settings of App1."}]}'::jsonb, '["slot1=B","slot2=A","slot3=E"]'::jsonb, 'TLS tương hỗ trong Azure App Service yêu cầu cấp Cơ bản, Tiêu chuẩn, Cao cấp hoặc Riêng biệt; D1 không được hỗ trợ. Sau khi bật xác thực chứng chỉ ứng dụng khách, hãy định cấu hình /public làm đường dẫn loại trừ chứng chỉ để nó không yêu cầu chứng chỉ ứng dụng khách, trong khi /private vẫn được bảo vệ. Xác thực nhà cung cấp danh tính không triển khai xác thực chứng chỉ ứng dụng khách TLS.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-198', 'az-204', 198, 'You need to add code at line PC26 of Processing.cs to ensure that security policies are met.  
  
How should you complete the code added at line PC26?', '[]'::jsonb, '[]'::jsonb, 'Mã hóa phía máy khách Azure Storage sử dụng `BlobEncryptionPolicy` được định cấu hình với `IKey` và `IKeyResolver`. Trình phân giải Key Vault chuyển đổi mã định danh của Gói khóa thành khóa bắt buộc và gán chính sách đó cho `DefaultRequestOptions.EncryptionPolicy` sẽ áp dụng chính sách đó cho các hoạt động blob-client.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure Azure Container Apps. Which features should you configure?', '[]'::jsonb, '[]'::jsonb, 'Nhãn sửa đổi cung cấp một URL duy nhất cho bản sửa đổi Azure Container Apps cụ thể và có thể được chuyển sang bản sửa đổi khác, giữ nguyên URL được người kiểm tra sử dụng. Chế độ sửa đổi kiểm soát những bản sửa đổi nào đang hoạt động; chế độ sửa đổi duy nhất kích hoạt bản sửa đổi mới và hướng lưu lượng truy cập đến nó khi nó sẵn sàng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-200', 'az-204', 200, 'You need to secure the Shipping Function app.  
  
How should the app be configured?', '[]'::jsonb, '[]'::jsonb, 'Hàm Azure được kích hoạt HTTP với cấp ủy quyền Hàm yêu cầu khóa truy cập dành riêng cho chức năng; Ẩn danh yêu cầu khóa truy cập no và Quản trị viên yêu cầu khóa chính của ứng dụng. JWT mang theo các xác nhận quyền sở hữu danh tính người dùng đã được xác thực, trong khi mã thông báo SAS ủy quyền quyền truy cập Bộ nhớ và khóa API không truyền tải các xác nhận quyền sở hữu của người dùng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you implement?', '[]'::jsonb, '[]'::jsonb, 'Thẻ chỉ mục Azure Blob là các thuộc tính khóa-giá trị được lập chỉ mục trên các đốm màu và hỗ trợ lọc phía máy chủ theo mã định danh khách hàng. Azure Cognitive Search có thể sử dụng bộ chỉ mục Azure Blob Storage để trích xuất và lập chỉ mục nội dung tài liệu để tìm kiếm toàn văn bản. Siêu dữ liệu Blob không phải là cơ chế lọc được lập chỉ mục phía máy chủ và các chính sách kiểm kê blob tạo ra các báo cáo thay vì tìm kiếm nội dung tài liệu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Provision the managed disk for maximum performance while satisfying these requirements.', '[]'::jsonb, '[]'::jsonb, 'SSD cao cấp cung cấp hiệu suất đĩa được quản lý cao nhất trong số các lựa chọn được liệt kê. Zone-redundant storage (ZRS) sao chép dữ liệu một cách đồng bộ trên availability zones trong một khu vực, cung cấp khả năng phục hồi khi mất điện ở trung tâm dữ liệu/khu vực. Ảnh chụp nhanh đĩa được quản lý theo thời gian có thể được giữ lại để khôi phục dữ liệu của ngày hôm trước.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which features should you use? Each feature may be used once, more than once, or not at all.', '{"statements":[{"id":"monitor_progress","text":"Monitor the progress of the change feed processor","correctAnswer":"A"},{"id":"prevent_batch_retry","text":"Prevent the change feed processor from retrying the entire batch when one document cannot be read","correctAnswer":"B"}],"choices":[{"key":"A","text":"Change feed estimator"},{"key":"B","text":"Dead-letter queue"},{"key":"C","text":"Deployment unit"},{"key":"D","text":"Lease container"}]}'::jsonb, '["monitor_progress=A","prevent_batch_retry=B"]'::jsonb, 'Công cụ ước tính nguồn cấp dữ liệu thay đổi theo dõi tiến trình của bộ xử lý bằng cách ước tính các thay đổi đang chờ xử lý trong nguồn cấp dữ liệu. Hàng đợi thư chết tồn tại một tài liệu riêng lẻ không thể xử lý được, cho phép bộ xử lý tiếp tục và tránh bị kẹt khi thử lại cùng một lô. Bộ chứa cho thuê lưu trữ trạng thái điểm kiểm tra/cho thuê, trong khi đơn vị triển khai mô tả một nhóm phiên bản bộ xử lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which configuration values should you use?', '[]'::jsonb, '[]'::jsonb, 'Trình xử lý tùy chỉnh Azure Functions hỗ trợ mã được viết bằng các ngôn ngữ mà Azure Functions không hỗ trợ nguyên bản khi mã có thể chạy máy chủ web và xử lý các nguyên gốc HTTP. Điều này sử dụng triển khai mã, ngăn xếp thời gian chạy Trình xử lý tùy chỉnh và lựa chọn phiên bản tùy chỉnh.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, '`az containerapp ingress traffic set` định cấu hình trọng số lưu lượng truy cập vào cho các bản sửa đổi Azure Container Apps. Hai trọng số sửa đổi được chỉ định, 80 và 20, tổng cộng là 100 phần trăm, do đó cả hai bản sửa đổi đều nhận được lưu lượng truy cập.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Use an App Service plan. Configure the Function App to use an Azure Blob Storage input trigger."},{"key":"B","text":"Use a Consumption plan. Configure the Function App to use an Azure Blob Storage trigger."},{"key":"C","text":"Use a Consumption plan. Configure the Function App to use a Timer trigger."},{"key":"D","text":"Use an App Service plan. Configure the Function App to use an Azure Blob Storage trigger."},{"key":"E","text":"Use a Consumption plan. Configure the Function App to use an Azure Blob Storage input trigger."}]'::jsonb, '["D"]'::jsonb, 'Trình kích hoạt Blob Storage chạy một hàm để phản hồi một blob mới hoặc được cập nhật, trong khi liên kết đầu vào Blob Storage chỉ cung cấp dữ liệu blob cho một hàm đã được gọi. Quá trình xử lý trình kích hoạt blob trên gói Tiêu thụ có thể bị trì hoãn khi ứng dụng không hoạt động. Gói Dịch vụ ứng dụng có thể sử dụng Luôn bật để tránh độ trễ không hoạt động đó, khiến gói này trở thành gói được liệt kê phù hợp để xử lý hình ảnh có độ trễ thấp.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which API should you use?', '[{"key":"A","text":"MongoDB API"},{"key":"B","text":"Table API"},{"key":"C","text":"SQL API"},{"key":"D","text":"Cassandra API"}]'::jsonb, '["C"]'::jsonb, 'API SQL Azure Cosmos DB hỗ trợ các truy vấn giống SQL và toán tử truy vấn quan hệ trên các tài liệu JSON, khiến nó trở thành API thích hợp để xử lý các lô dữ liệu quan hệ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How many alerts should you expect?', '[{"key":"A","text":"1"},{"key":"B","text":"2"},{"key":"C","text":"3"},{"key":"D","text":"4"}]'::jsonb, '["A"]'::jsonb, 'Cảnh báo về tính khả dụng của Application Insights dựa trên trạng thái: một cảnh báo được tạo khi phát hiện điểm cuối là không khả dụng và các cảnh báo tiếp theo của no được tạo ở các lần đánh giá sau này trong khi nó vẫn không khả dụng. Tần suất kiểm tra 15 phút không gây ra cảnh báo lặp đi lặp lại trong cùng 50 phút ngừng hoạt động.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Azure Monitor logs"},{"key":"B","text":"Application Insights alerts"},{"key":"C","text":"Azure Monitor metrics"},{"key":"D","text":"Application Insights web tests"}]'::jsonb, '["A","D"]'::jsonb, 'Để phân tích thời gian hoạt động hàng tháng, bạn cần có dữ liệu sẵn có cùng với cách truy vấn dữ liệu đó theo thời gian. Các bài kiểm tra tính khả dụng của Application Insights (ping web/URL) (D) chủ động thăm dò ứng dụng từ nhiều vị trí và ghi lại kết quả về tính khả dụng, đây là cơ chế tiêu chuẩn để đo thời gian hoạt động của ứng dụng đa khu vực, có giao diện Trình quản lý lưu lượng truy cập. Nhật ký Azure Monitor (A) lưu trữ các Kết quả sẵn có đó và cho phép bạn chạy các truy vấn KQL để tổng hợp thời gian hoạt động mỗi tháng. Chỉ số Azure Monitor (C) lưu giữ dữ liệu trong một khoảng thời gian giới hạn và không phù hợp với phân tích lịch sử hàng tháng, đồng thời cảnh báo Application Insights (B) chỉ thông báo về các điều kiện thay vì phân tích thời gian hoạt động, vì vậy mỗi chỉ số đều là một giải pháp chưa hoàn chỉnh.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Run the Add-AzKeyVaultKey cmdlet."},{"key":"B","text":"Run the az keyvault update --enable-soft-delete true --enable-purge-protection true CLI."},{"key":"C","text":"Implement virtual network service endpoints for Azure Key Vault."},{"key":"D","text":"Run the az keyvault update --enable-soft-delete false CLI."}]'::jsonb, '["B"]'::jsonb, 'Xóa mềm Azure Key Vault giữ lại các vault và đối tượng vault đã xóa trong khoảng thời gian lưu giữ có thể phục hồi, mặc định là 90 ngày. Bảo vệ thanh lọc ngăn chặn việc thanh lọc vĩnh viễn trong thời gian đó. Việc kích hoạt cả hai tính năng này sẽ cung cấp khả năng bảo vệ khôi phục cho các kho tiền, khóa, bí mật và chứng chỉ vô tình bị xóa.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-221', 'az-204', 221, 'You need to fix the corporate website error.  
  
Which four actions should be recommended, in sequence?', '{"statements":[{"id":"step1","text":"Slot 1:","correctAnswer":"G"},{"id":"step2","text":"Slot 2:","correctAnswer":"F"},{"id":"step3","text":"Slot 3:","correctAnswer":"D"},{"id":"step4","text":"Slot 4:","correctAnswer":"C"}],"choices":[{"key":"A","text":"Upload the certificate to Azure Key Vault."},{"key":"B","text":"Update line SC05 of Security.cs to include error handling and then redeploy the code."},{"key":"C","text":"Update line SC03 of Security.cs to include a using statement and then re-deploy the code."},{"key":"D","text":"Add the certificate thumbprint to the WEBSITE_LOAD_CERTIFICATES app setting."},{"key":"E","text":"Upload the certificate to source control."},{"key":"F","text":"Import the certificate to Azure App Service."},{"key":"G","text":"Generate a certificate."}]}'::jsonb, '["step1=G","step2=F","step3=D","step4=C"]'::jsonb, 'Chứng chỉ Dịch vụ ứng dụng phải được tạo và nhập vào ứng dụng trước khi có thể sử dụng. WEBSITE_LOAD_CERTIFICATES cung cấp chứng chỉ được chỉ định bằng dấu vân tay của nó cho mã ứng dụng. Mã lưu trữ chứng chỉ C# yêu cầu không gian tên chứng chỉ X.509, sau đó ứng dụng đã cập nhật phải được triển khai lại.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the Azure Resource Manager template?', '[]'::jsonb, '[]'::jsonb, '`copyIndex()` trả về chỉ mục lặp cho vòng lặp sao chép tài nguyên, cho phép mỗi tên tài khoản lưu trữ là duy nhất. Đối tượng `copy` xác định vòng lặp tài nguyên đó và số lượng của nó. `dependsOn` đặt thứ tự triển khai một cách rõ ràng, do đó tài nguyên máy ảo sẽ chờ bộ cân bằng tải, mạng ảo và tài nguyên tài khoản lưu trữ cần thiết.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to ensure the blob is encrypted. How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, 'Khóa do khách hàng cung cấp Azure Blob Storage cho phép ứng dụng cung cấp khóa mã hóa AES-256 riêng cho các hoạt động blob. `CustomerProvidedKey` được định cấu hình trên `BlobClientOptions`; xác thực giá trị khóa trước khi đặt tùy chọn đó sẽ tránh tiếp tục với khóa không đạt giá trị xác minh của ứng dụng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Add the customer ID for the signed in user to the CorrelationContext in the web application"},{"key":"B","text":"On the current SpanContext, set the TraceId to the customer ID for the signed in user"},{"key":"C","text":"Set the header Ocp-Apim-Trace to the customer ID for the signed in user"},{"key":"D","text":"Create a new SpanContext with the TraceFlags value set to the customer ID for the signed in user"}]'::jsonb, '["A"]'::jsonb, 'CorrelationContext truyền các thuộc tính theo dõi phân phối tên-giá trị do ứng dụng xác định bằng một thao tác, do đó, việc đặt ID khách hàng của người dùng đã đăng nhập vào đó sẽ liên kết giá trị đó với phép đo từ xa và các hoạt động HTTP xuôi dòng. ID theo dõi và cờ theo dõi là các trường theo dõi dành riêng, không phải là nơi chứa số nhận dạng doanh nghiệp.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, '`GetChanges(x).AsPages()` liệt kê nguồn cấp dữ liệu thay đổi dưới dạng trang, sử dụng `x` làm mã thông báo tiếp tục khi tiếp tục. `ContinuationToken` của mỗi trang xác định nơi bắt đầu liệt kê tiếp theo, trong khi `Values` chứa các sự kiện cần xử lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to implement user properties in the web app. Which code library and API should you use?', '[]'::jsonb, '[]'::jsonb, 'Microsoft Graph hiển thị tài nguyên người dùng Azure AD B2C và thuộc tính hồ sơ của họ. MSAL cung cấp xác thực ứng dụng và thu thập mã thông báo cho Azure AD B2C, cho phép ứng dụng web gọi Microsoft Graph với các quyền được ủy quyền phù hợp.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which command should you use?', '[{"key":"A","text":"Get-AzureADUser"},{"key":"B","text":"Get-AzResource"},{"key":"C","text":"Get-AzureADUserOwnedObject"},{"key":"D","text":"az ad signed-in-user list-owned-objects"}]'::jsonb, '["B"]'::jsonb, 'Đối với system-assigned managed identity, ID đối tượng của managed identity service principal là `PrincipalId` của nhận dạng tài nguyên. `Get-AzResource` truy xuất tài nguyên VM, cho phép lấy giá trị `Identity.PrincipalId` của nó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Create a collection sharded on username to store documents."},{"key":"B","text":"Configure Azure Cosmos DB to use the Gremlin API."},{"key":"C","text":"Create an unsharded collection to store documents."},{"key":"D","text":"Configure Azure Cosmos DB to use the MongoDB API."}]'::jsonb, '["C","D"]'::jsonb, 'Azure Cosmos DB dành cho MongoDB cung cấp các giao dịch nhiều tài liệu với ngữ nghĩa ACID chỉ trong một bộ sưu tập không được phân chia. Các giao dịch nhiều tài liệu không được hỗ trợ trong các bộ sưu tập được phân chia, vì vậy cấu hình bắt buộc là một bộ sưu tập không được phân chia bằng API MongoDB.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which three Azure CLI commands should you use, in the correct order?', '{"statements":[{"id":"step1","text":"Slot 1:","correctAnswer":"A"},{"id":"step2","text":"Slot 2:","correctAnswer":"E"},{"id":"step3","text":"Slot 3:","correctAnswer":"D"}],"choices":[{"key":"A","text":"az group create"},{"key":"B","text":"az group update"},{"key":"C","text":"az webapp update"},{"key":"D","text":"az webapp create"},{"key":"E","text":"az appservice plan create"}]}'::jsonb, '["step1=A","step2=E","step3=D"]'::jsonb, 'Tạo nhóm tài nguyên ở vị trí Dịch vụ ứng dụng hỗ trợ Linux, sau đó tạo gói Dịch vụ ứng dụng Linux trong nhóm đó (sử dụng `--is-linux`). Tạo ứng dụng web trong gói đó bằng `az webapp create`, chỉ định hình ảnh vùng chứa cho lần triển khai ban đầu. Các lệnh cập nhật sửa đổi các tài nguyên hiện có và không phù hợp để cung cấp các tài nguyên này.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure the application settings so diagnostic data is stored as required.', '[]'::jsonb, '[]'::jsonb, 'Các vùng chứa tùy chỉnh Linux trong Azure App Service chỉ duy trì dữ liệu trong `/home` khi bộ lưu trữ Dịch vụ ứng dụng được bật. Việc đặt `WEBSITE_ENABLE_APP_SERVICE_STORAGE` thành `true` sẽ bật tính năng lưu trữ đó và việc đặt `DIAGDATA` thành `/home` sẽ hướng mã ghi dữ liệu chẩn đoán vào đó.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783790037738-uxr7jtrs.jpg', 'published'),
...
> **NOTE:** Each correct selection is worth one point.', '[{"key":"A","text":"Peek at a message in the queue."},{"key":"B","text":"Delete a message in the queue."},{"key":"C","text":"Add a message to the queue."},{"key":"D","text":"Update a message in the queue."},{"key":"E","text":"Receive a message from the queue."}]'::jsonb, '["D","E"]'::jsonb, 'Hoạt động Nhận/Nhận tin nhắn chấp nhận tham số `visibilitytimeout`, làm cho mỗi tin nhắn được truy xuất trở nên ẩn đối với những người tiêu dùng khác trong khoảng thời gian được chỉ định. Hoạt động Cập nhật tin nhắn thay đổi rõ ràng thời gian chờ hiển thị của một tin nhắn được xác định, sử dụng ID tin nhắn và biên nhận pop của nó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to implement the message.', '[]'::jsonb, '[]'::jsonb, 'CloudStorageAccount.CreateCloudQueueClient tạo CloudQueueClient. GetQueueReference truy xuất đối tượng CloudQueue theo tên hàng đợi. CloudQueue hỗ trợ tạo hàng đợi không đồng bộ và thêm CloudQueueMessage.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-236', 'az-204', 236, 'You must support the requirements of the Shipping Logic App.  
  
What should you use?', '[{"key":"A","text":"Azure Active Directory Application Proxy"},{"key":"B","text":"Site-to-Site (S2S) VPN connection"},{"key":"C","text":"On-premises Data Gateway"},{"key":"D","text":"Point-to-Site (P2S) VPN connection"}]'::jsonb, '["C"]'::jsonb, 'Cổng dữ liệu tại chỗ cung cấp cầu nối an toàn được các trình kết nối Ứng dụng Azure Logic sử dụng để truy cập các nguồn dữ liệu tại chỗ. Nó hỗ trợ giao tiếp gửi đi từ môi trường tại chỗ và tránh yêu cầu kết nối gửi đến.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to provide a script for key rotation. How should you complete the command?', '[]'::jsonb, '[]'::jsonb, 'Mã hóa do khách hàng quản lý của Azure Storage sử dụng khóa Key Vault. `az keyvault key list-versions` truy xuất mã nhận dạng phiên bản cho khóa đó và `Microsoft.Keyvault` định cấu hình tài khoản lưu trữ để sử dụng nguồn khóa do khách hàng Key Vault quản lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this accomplish the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Việc triển khai tệp kê khai AKS yêu cầu `kubectl` được định cấu hình bằng thông tin xác thực cụm, thường thu được bằng `az aks get-credentials` và triển khai bằng `kubectl apply -f <manifest>`. Chỉ khởi động vùng chứa Azure CLI sẽ không định cấu hình quyền truy cập Kubernetes cũng như không áp dụng tệp kê khai YAML.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-239', 'az-204', 239, 'Solution: You install the Azure CLI on the device and run the `kubectl apply "f myapp.yaml` command.  
  
Does this achieve the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Tệp kê khai YAML được triển khai với kubectl bằng cách sử dụng cờ `-f`, chẳng hạn như `kubectl apply -f myapp.yaml`. Riêng Azure CLI không cài đặt kubectl và lệnh hiển thị không sử dụng cú pháp cờ tệp kê khai hợp lệ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure routing for the web app.', '[]'::jsonb, '[]'::jsonb, 'Quy tắc lộ trình được đánh giá theo thứ tự khai báo và quá trình xử lý dừng ở lần khớp đầu tiên. Do đó, quy tắc `/manuals/new.html` chính xác phải đứng trước `/manuals*`; nó giới hạn trang đó ở `contributors`. Sau đó, quy tắc ký tự đại diện cho phép bất kỳ người dùng được xác thực nào truy cập vào các đường dẫn `/manuals` còn lại. `authenticated` là vai trò tích hợp dành cho người dùng đã đăng nhập.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure the application''s manifest to meet the authentication requirements.', '[]'::jsonb, '[]'::jsonb, '`groupMembershipClaims` định cấu hình xác nhận quyền sở hữu nhóm được phát trong mã thông báo truy cập của người dùng hoặc OAuth 2.0; `"All"` bao gồm các nhóm bảo mật, nhóm phân phối và vai trò thư mục của người dùng. `oauth2AllowImplicitFlow` kích hoạt mã thông báo truy cập luồng ngầm cho ứng dụng dựa trên trình duyệt.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you validate?', '[{"key":"A","text":"ID token header"},{"key":"B","text":"ID token signature"},{"key":"C","text":"HTTP response code"},{"key":"D","text":"Tenant ID"}]'::jsonb, '["B"]'::jsonb, 'Mã thông báo ID là JWT đã ký, do đó, việc xác thực chữ ký của nó bằng khóa ký được xuất bản của nhà cung cấp danh tính sẽ xác minh rằng mã thông báo được phát hành bởi nhà cung cấp đáng tin cậy và không bị thay đổi. Xác thực mã thông báo cũng sẽ xác thực các yêu cầu có thể áp dụng, nhưng chữ ký là xác thực có liên quan trong số các lựa chọn này.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Each component may be used once, more than once, or not at all.', '{"statements":[{"id":"app1","text":"App1","correctAnswer":"C"},{"id":"app2","text":"App2","correctAnswer":"A"}],"choices":[{"key":"A","text":"Lease container"},{"key":"B","text":"Integrated cache"},{"key":"C","text":"Continuation token"}]}'::jsonb, '["app1=C","app2=A"]'::jsonb, 'Mô hình kéo yêu cầu khách hàng lưu mã thông báo tiếp tục làm dấu trang cho những thay đổi được xử lý cuối cùng. Mô hình đẩy sử dụng bộ xử lý nguồn cấp dữ liệu thay đổi, bộ xử lý này tự động lưu trữ trạng thái xử lý của nó trong vùng chứa cho thuê. Bộ đệm tích hợp không liên quan đến việc theo dõi điểm kiểm tra nguồn cấp dữ liệu thay đổi.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do first?', '[{"key":"A","text":"Configure delivery properties of EventSub1."},{"key":"B","text":"Create an Azure Blob Storage container in Sub1."},{"key":"C","text":"Create an Azure Storage queue in Sub1."},{"key":"D","text":"Configure the retry policy of EventSub1."}]'::jsonb, '["B"]'::jsonb, 'Thư chết Azure Event Grid lưu trữ các sự kiện không thể gửi được trong vùng chứa Azure Blob Storage. Do đó, tài khoản lưu trữ và vùng chứa blob của nó phải được tạo trước khi định cấu hình đích thư chết của đăng ký sự kiện. [Set dead-letter location and retry policy — Azure Event Grid](https://learn.microsoft.com/en-us/azure/event-grid/manage-event-delivery)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Use a Put Block List operation with a request header of x-ms-immutability-policy-mode."},{"key":"B","text":"Create a lifecycle policy with an action of tierToArchive and configure daysAfterModificationGreaterThan for 0."},{"key":"C","text":"Use a Put Blob operation with a request header of x-ms-immutability-policy-until-date."},{"key":"D","text":"Create a lifecycle policy with an action of tierToArchive and configure a filter for blobIndexMatch."}]'::jsonb, '["B"]'::jsonb, 'Quản lý vòng đời Azure Blob Storage có thể chuyển các khối khối đủ điều kiện sang tầng truy cập lưu trữ thông qua hành động `tierToArchive`. Việc đặt `daysAfterModificationGreaterThan` thành `0` làm cho các đốm màu đủ điều kiện ngay sau lần sửa đổi cuối cùng, bao gồm cả việc sao chép dữ liệu vào Blob Storage. Tiêu đề chính sách bất biến không đặt cấp truy cập và bộ lọc chỉ mục blob chỉ xác định blob nào mà quy tắc vòng đời nhắm mục tiêu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which of the following options should you use?', '[{"key":"A","text":"The Activity log signal type."},{"key":"B","text":"The Application Log signal type."},{"key":"C","text":"The Metric signal type."},{"key":"D","text":"The Audit Log signal type."}]'::jsonb, '["C"]'::jsonb, 'Quy tắc cảnh báo số liệu Azure Monitor hỗ trợ các thứ nguyên để theo dõi chuỗi thời gian riêng biệt và có thể được định cấu hình ở trạng thái. Cảnh báo số liệu có trạng thái sẽ gửi thông báo khi kích hoạt và một thông báo khác khi điều kiện được giải quyết. Cảnh báo Nhật ký hoạt động giám sát các sự kiện trong nhật ký hoạt động của Azure thay vì các số liệu về bộ đếm hiệu suất.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Configure and apply header-based versioning."},{"key":"B","text":"Create and publish a product."},{"key":"C","text":"Configure and apply query string-based versioning."},{"key":"D","text":"Add a new revision to all APIs. Make the revisions current and add a change log entry."}]'::jsonb, '["B"]'::jsonb, 'Sản phẩm Quản lý API Azure nhóm các API và cung cấp các biện pháp kiểm soát đăng ký cấp sản phẩm. Một sản phẩm được xuất bản, được bảo vệ có thể yêu cầu khóa đăng ký, đưa ra các điều khoản sử dụng để người đăng ký chấp nhận, yêu cầu quản trị viên phê duyệt các yêu cầu đăng ký và đặt giới hạn cho các đăng ký đồng thời.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to complete the configuration of the stored access policy and generate the shared access signature token.', '[]'::jsonb, '[]'::jsonb, 'Chính sách truy cập được lưu trữ xác định quyền, thời gian bắt đầu và thời gian hết hạn cho SAS dịch vụ. Chữ ký truy cập chia sẻ vùng chứa Blob là SAS dịch vụ và có thể sử dụng chính sách truy cập được lưu trữ; tài khoản SAS và ủy quyền người dùng SAS không thể.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Each SAS type may be used once, more than once, or not at all.', '{"statements":[{"id":"slot1","text":"Delegate access to resources in one or more of the storage services","correctAnswer":"A"},{"id":"slot2","text":"Delegate access to a resource in a single storage service","correctAnswer":"B"},{"id":"slot3","text":"Secure a resource by using Azure AD credentials","correctAnswer":"C"}],"choices":[{"key":"A","text":"Account-level"},{"key":"B","text":"Service-level"},{"key":"C","text":"User delegation"}]}'::jsonb, '["slot1=A","slot2=B","slot3=C"]'::jsonb, 'Một tài khoản SAS ủy quyền truy cập trên nhiều dịch vụ lưu trữ, trong khi SAS dịch vụ được giới hạn trong một tài nguyên trong một dịch vụ lưu trữ. SAS ủy quyền người dùng được ký bằng thông tin xác thực Microsoft Entra ID (Azure AD) và được sử dụng để ủy quyền quyền truy cập được bảo mật bằng các thông tin xác thực đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Restart all apps in the App Service plan."},{"key":"B","text":"Enable Application Insights site extensions."},{"key":"C","text":"Upgrade the Azure App Service plan to Premium."},{"key":"D","text":"Enable Profiler."},{"key":"E","text":"Enable the Always On setting for the app service."},{"key":"F","text":"Enable Snapshot debugger."}]'::jsonb, '["B","D","E"]'::jsonb, 'Application Insights Profiler ghi lại các ngăn xếp cuộc gọi đối với các yêu cầu chậm và lưu trữ kết quả trong Application Insights, cho phép phân tích trên các phiên bản Dịch vụ ứng dụng. Tiện ích mở rộng trang web Application Insights cung cấp tích hợp đo từ xa Dịch vụ ứng dụng và Luôn bật phải được bật cho Profiler. Profiler được hỗ trợ ở cấp Dịch vụ ứng dụng cơ bản trở lên, do đó, việc nâng cấp Premium là không cần thiết và sẽ làm tăng thêm chi phí.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you use?', '[{"key":"A","text":"Secret versioning"},{"key":"B","text":"Azure Policy"},{"key":"C","text":"Key Vault Firewall"},{"key":"D","text":"Access policies"}]'::jsonb, '["B"]'::jsonb, 'Chính sách Azure có thể thực thi các yêu cầu về khóa Key Vault thông qua các chính sách hạn chế các loại khóa mật mã được phép và yêu cầu kích thước khóa RSA tối thiểu được chỉ định, sử dụng các hiệu ứng như Từ chối hoặc Kiểm tra.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
For each of the following statements, select **Yes** if the statement is true. Otherwise, select **No**.', '{"statements":[{"id":"1","text":"A log alert is created that sends an email when the CPU percentage is above 60 percent for five minutes.","correctAnswer":"No"},{"id":"2","text":"A log alert is created that sends an email when the number of virtual machine heartbeats in the past hour is less than five.","correctAnswer":"Yes"},{"id":"3","text":"The log alert is scheduled to run every two hours.","correctAnswer":"No"}],"choices":[{"key":"Yes","text":"Yes"},{"key":"No","text":"No"}]}'::jsonb, '["1:No","2:Yes","3:No"]'::jsonb, 'Quy tắc đánh giá các bản ghi Nhịp tim từ giờ trước và kích hoạt khi số lượng bản ghi đó nhỏ hơn năm. Nhóm hành động cung cấp thông báo qua email. Tần suất 60 phút chạy quy tắc mỗi giờ và điều kiện phần trăm CPU no được định cấu hình.', 'General', '["AZ-204","General"]'::jsonb, 'https://cdn.examcademy.com/images/questions/1783792688926-w4vfp3ow.png', 'published'),
...
Which command should you run?', '[{"key":"A","text":"az storage blob tag set"},{"key":"B","text":"az storage blob service-properties update"},{"key":"C","text":"az storage blob directory metadata update"},{"key":"D","text":"New-AzStorageBlobQueryConfig"}]'::jsonb, '["A"]'::jsonb, 'Thẻ chỉ mục Azure Blob lưu trữ các cặp khóa-giá trị trên các đốm màu riêng lẻ và được lập chỉ mục tự động để tìm kiếm và lọc. Lệnh Azure CLI đặt các thẻ này là `az storage blob tag set`.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["A"]'::jsonb, 'Vai trò ứng dụng Microsoft Entra ID hỗ trợ ủy quyền dựa trên vai trò: vai trò có thể được xác định cho một ứng dụng, được gán cho các nhóm và được bao gồm trong mã thông báo của người dùng đã đăng nhập thông qua xác nhận quyền sở hữu `roles`. Ứng dụng có thể ủy quyền cho người dùng dựa trên yêu cầu đó, ánh xạ tư cách thành viên nhóm Azure AD tới các cấp quyền được yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution achieve the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Việc yêu cầu xác thực Microsoft Entra ID sẽ chặn các yêu cầu chưa được xác thực nhưng không ánh xạ tư cách thành viên nhóm Azure AD tới cấp độ quyền quản trị viên, quyền thông thường và quyền người đọc. Ủy quyền theo vai trò hoặc nhóm cụ thể phải đánh giá các khiếu nại của người dùng được xác thực.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Xác thực Azure App Service cho người dùng đăng nhập bằng thông tin xác thực Azure AD (Microsoft Entra ID) phải được đặt cấu hình với Microsoft Entra ID làm nhà cung cấp danh tính. Xác thực Windows tích hợp được hỗ trợ cho các máy khách di động và máy tính để bàn Windows được tham gia miền hoặc được tham gia vào miền hiện hành, không phải như cấu hình xác thực web Dịch vụ ứng dụng bắt buộc. Truy vấn Microsoft Graph về tư cách thành viên nhóm có thể hỗ trợ ủy quyền, nhưng nó không làm cho phương pháp xác thực đáp ứng được yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which availability test should you use?', '[{"key":"A","text":"standard test"},{"key":"B","text":"multi-step web test"},{"key":"C","text":"custom TrackAvailability test"},{"key":"D","text":"URL ping test"}]'::jsonb, '["C"]'::jsonb, 'Các thử nghiệm về tính khả dụng của Application Insights công khai yêu cầu điểm cuối HTTP hoặc HTTPS có thể truy cập được từ Internet công cộng. Đối với điểm cuối nội bộ hoặc điểm cuối xâm nhập no, việc kiểm tra tính khả dụng phải chạy từ bên trong mạng và gửi kết quả đến Application Insights thông qua API `TrackAvailability()`. Điều này giữ lại quyền truy cập internet bị chặn của trang web.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Create an Azure App Service API and implement the change feed estimator of the SDK. Scale the API by using multiple Azure App Service instances."},{"key":"B","text":"Create a background job in an Azure Kubernetes Service and implement the change feed feature of the SDK."},{"key":"C","text":"Create an Azure Function to use a trigger for Azure Cosmos DB. Configure the trigger to connect to the container."},{"key":"D","text":"Create an Azure Function that uses a FeedIterator object that processes the change feed by using the pull model on the container. Use a FeedRange object to parallelize the processing of the change feed across multiple functions."}]'::jsonb, '["C","D"]'::jsonb, 'Trình kích hoạt Azure Cosmos DB cho Azure Functions sử dụng bộ xử lý nguồn cấp dữ liệu thay đổi bên trong, phát hiện các phần chèn và cập nhật trong vùng chứa được giám sát, đồng thời tự động xử lý song song trên các phân vùng vùng chứa. Mô hình kéo nguồn cấp dữ liệu thay đổi cũng có thể xử lý các thay đổi song song bằng cách lấy các giá trị FeedRange và sử dụng FeedIterator cho từng phạm vi trên nhiều phiên bản hàm; mã chức năng có thể ghi từng thay đổi được xử lý vào Azure Blob Storage. Công cụ ước tính nguồn cấp dữ liệu thay đổi báo cáo công việc chưa hoàn thành nhưng không sử dụng hoặc xử lý nguồn cấp dữ liệu thay đổi.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which code segment should you use?', '[{"key":"A","text":"new Container(EndpointUri, PrimaryKey);"},{"key":"B","text":"new Database(EndpointUri, PrimaryKey);"},{"key":"C","text":"new CosmosClient(EndpointUri, PrimaryKey);"}]'::jsonb, '["C"]'::jsonb, '`CosmosClient` là ứng dụng khách cấp tài khoản của Azure Cosmos DB.NET SDK để định cấu hình và thực hiện các yêu cầu. Hàm tạo của nó chấp nhận URI điểm cuối tài khoản và khóa tài khoản; `Database` và `Container` là các đối tượng tài nguyên được truy cập thông qua ứng dụng khách đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which configuration should you use?', '[{"key":"A","text":"Set the partition key of messages to the message ID value."},{"key":"B","text":"Enable sessions. Set the partition key of messages to the session ID value."},{"key":"C","text":"Enable sessions. Ensure that the partition key is different from the session ID value."},{"key":"D","text":"Leave the partition key value as null."}]'::jsonb, '["D"]'::jsonb, 'Việc không đặt khóa phân vùng cho phép Azure Service Bus phân phối thông báo giữa các phân vùng và thử lại phân vùng khác khi phân vùng được chọn ban đầu tạm thời không khả dụng. Khóa phân vùng được đặt, bao gồm ID phiên, ghim tin nhắn vào một phân vùng và có thể gây ra lỗi nếu phân vùng đó không khả dụng. Các phiên là không cần thiết vì không cần phải giao hàng theo yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Create an action group."},{"key":"B","text":"Create an alert rule."},{"key":"C","text":"Enable the test."},{"key":"D","text":"Enable the alert."}]'::jsonb, '["A"]'::jsonb, 'Kiểm tra tính khả dụng của Application Insights có quy tắc cảnh báo tính khả dụng liên quan nhưng việc gửi email được định cấu hình thông qua nhóm hành động Azure Monitor. Một nhóm hành động có thể xác định thông báo qua email cho vai trò **Chủ sở hữu** Trình quản lý tài nguyên Azure ở phạm vi đăng ký, do đó, việc đính kèm nhóm hành động đó vào cảnh báo về tính khả dụng sẽ gửi thông báo khi thử nghiệm thất bại.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Enable the change feed on the storage account to begin capturing and recording changes."},{"key":"B","text":"Configure object replication and specify replication rules."},{"key":"C","text":"Create a snapshot of the blob in the hot tier."},{"key":"D","text":"Configure an immutability policy that is scoped to a blob version."}]'::jsonb, '["A"]'::jsonb, 'Khôi phục theo thời điểm Azure Blob Storage yêu cầu xóa mềm blob, lập phiên bản blob và nguồn cấp dữ liệu thay đổi. Khi tính năng xóa mềm và lập phiên bản đã được bật, tài khoản lưu trữ phải kích hoạt nguồn cấp dữ liệu thay đổi để nắm bắt các thay đổi blob cần thiết nhằm khôi phục các blob khối về thời điểm trước đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which command should you run?', '[{"key":"A","text":"Update -AzStorageBobServiceProperty"},{"key":"B","text":"Set-AzStorageBlobTag"},{"key":"C","text":"az storage blob service-properties update"},{"key":"D","text":"Set-AzStorageBlobContent"}]'::jsonb, '["B"]'::jsonb, 'Thẻ chỉ mục Azure Blob là thuộc tính khóa-giá trị mà Azure Storage tự động lập chỉ mục, cho phép định vị các đốm màu bằng biểu thức thẻ. `Set-AzStorageBlobTag` gán các thẻ chỉ mục blob đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Marketing needs a view showing the number of sales for every unique ad. Implement the query for the view.', '[]'::jsonb, '[]'::jsonb, '`count(c.whenFinished)` trả về số lượng bán hàng kèm theo ngày hoàn thành được điền sẵn. `DateTimeBin(c.whenFinished, ''day'', 2)` nhóm các dấu thời gian thành các khoảng thời gian hai ngày liên tiếp, phù hợp với các quảng cáo thay đổi mỗi ngày. Truy vấn tổng hợp được nhóm phải chọn và nhóm theo cùng một biểu thức bin.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-288', 'az-204', 288, 'Solution: Create an Azure Event Grid. Configure event filtering to evaluate the device identifier.  
  
Does this solution meet the goal?', '[{"key":"A","text":"Yes"},{"key":"B","text":"No"}]'::jsonb, '["B"]'::jsonb, 'Azure Event Grid lọc và định tuyến các sự kiện đến các trình xử lý đã đăng ký; bộ lọc nâng cao có thể đánh giá các trường trong dữ liệu sự kiện. Bản thân nó không cung cấp khả năng nhập dữ liệu từ xa của thiết bị hoặc lưu trữ tải trọng POS trong Azure Blob Storage, do đó việc định cấu hình bộ lọc nhận dạng thiết bị không đáp ứng yêu cầu lưu trữ và nhận dữ liệu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you use?', '[{"key":"A","text":"Azure Storage Queue"},{"key":"B","text":"Azure Event Hub"},{"key":"C","text":"Azure Service Bus"},{"key":"D","text":"Azure Event Grid"}]'::jsonb, '["C"]'::jsonb, 'Azure Service Bus được thiết kế để nhắn tin giao dịch và doanh nghiệp đáng tin cậy giữa các dịch vụ. Các phiên tin nhắn của nó cung cấp khả năng xử lý FIFO được đảm bảo cho các chuỗi tin nhắn liên quan, khiến nó phù hợp với bảng nối đa năng liên lạc vi dịch vụ được đặt hàng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Copy blobs to Container2 by using the Put Blob operation of the Blob Service REST API"},{"key":"B","text":"Create an Event Grid topic that uses the Start-AzureStorageBlobCopy cmdlet"},{"key":"C","text":"Use AzCopy with the Snapshot switch to copy blobs to Container2"},{"key":"D","text":"Download the blob to a virtual machine and then upload the blob to Container2"}]'::jsonb, '["B"]'::jsonb, 'Azure Blob Storage phát ra sự kiện Lưới sự kiện `Microsoft.Storage.BlobCreated` khi một blob được tạo hoặc thay thế thông qua API Blob REST. Trình xử lý được kích hoạt bởi Lưới sự kiện có thể bắt đầu bản sao blob phía máy chủ cho video đã tải lên đủ điều kiện, do đó việc sao chép diễn ra để phản hồi lại mỗi lần tải lên bất thường mà không cần thăm dò ý kiến ​​hoặc chuyển blob thông qua máy ảo.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you configure for AI1?', '[]'::jsonb, '[]'::jsonb, 'Một nhóm thuần tập xác định một nhóm người dùng có thể sử dụng lại dựa trên tiêu chí hành vi, bao gồm số lượt xem trang và số lần xuất hiện tối thiểu trong một khoảng thời gian. Application Insights sử dụng KQL để truy vấn và phân tích phép đo từ xa như phiên và sự kiện.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Process all Azure Blob storage events by using Azure Event Grid with a subscriber Azure Function app."},{"key":"B","text":"Enable the change feed on the storage account and process all changes for available events."},{"key":"C","text":"Process all Azure Storage Analytics logs for successful blob events."},{"key":"D","text":"Use the Azure Monitor HTTP Data Collector API and scan the request body for successful blob events."}]'::jsonb, '["B"]'::jsonb, 'Nguồn cấp dữ liệu thay đổi Azure Blob Storage cung cấp nhật ký giao dịch bền vững, không thay đổi, chỉ đọc để thay đổi siêu dữ liệu blob và blob. Các ứng dụng có thể sử dụng nguồn cấp dữ liệu không đồng bộ, tăng dần hoặc theo đợt và việc lưu giữ có thể được định cấu hình để đáp ứng các yêu cầu tuân thủ. Nguồn cấp dữ liệu ghi lại các sự kiện thay đổi blob có sẵn, cho phép người tiêu dùng lọc theo các loại hoạt động được yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the PowerShell segment?', '[]'::jsonb, '[]'::jsonb, 'system-assigned managed identity được gắn với vòng đời của VM và bị xóa khi VM bị xóa. user-assigned managed identity là tài nguyên Azure riêng biệt có vòng đời độc lập. `-AssignIdentity` gán danh tính hệ thống cho VM.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which locations should you configure? Each location may be used once, more than once, or not at all.', '{"statements":[{"id":"read","text":"Read","correctAnswer":"B"},{"id":"write","text":"Write","correctAnswer":"C"}],"choices":[{"key":"A","text":"db1"},{"key":"B","text":"app1"},{"key":"C","text":"account1"}]}'::jsonb, '["read=B","write=C"]'::jsonb, 'Ứng dụng khách SDK có thể ghi đè mức độ nhất quán mặc định của tài khoản đối với các yêu cầu đọc riêng lẻ. Việc ghi đè chỉ áp dụng cho các lần đọc; tính nhất quán ghi vẫn bị chi phối bởi mức độ nhất quán mặc định được đặt trên tài khoản Azure Cosmos DB.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
> **NOTE:** Each correct selection is worth one point.', '[{"key":"A","text":"Orchestrator"},{"key":"B","text":"Entity"},{"key":"C","text":"Client"},{"key":"D","text":"Activity"}]'::jsonb, '["A","D"]'::jsonb, 'Chức năng điều phối điều phối quy trình đặt hàng lâu dài và gọi các chức năng hoạt động. Các hàm hoạt động là nơi thích hợp cho I/O bên ngoài, chẳng hạn như gọi API bên ngoài để truy xuất dữ liệu giảm giá, vì các hàm điều phối thông thường không được thực hiện I/O trực tiếp.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to use the image Exif data as blob metadata in the application. Which HTTP verbs should you use?', '[]'::jsonb, '[]'::jsonb, 'Siêu dữ liệu Blob được viết bằng PUT. Yêu cầu HEAD truy xuất các thuộc tính blob và siêu dữ liệu do người dùng xác định từ tiêu đề mà không trả về nội dung blob, giảm thiểu băng thông và thời gian xử lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the code segment?', '[]'::jsonb, '[]'::jsonb, '`DaprClientBuilder().Build()` tạo `DaprClient`, cung cấp `InvokeMethodAsync` cho lệnh gọi dịch vụ Dapr. Gọi nó bằng ID ứng dụng PaymentService (`paymentservice`) và `ProcessPayment` định tuyến yêu cầu thông qua sidecar Dapr tới phương thức dịch vụ đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-307', 'az-204', 307, 'Add code at line AM09 so that users can review content through ContentAnalysisService. How should the code be completed?', '[]'::jsonb, '[]'::jsonb, 'React SPA dựa trên trình duyệt phải được bật để nhận cả mã thông báo truy cập luồng ngầm OAuth 2.0 và mã thông báo ID. `oauth2AllowImplicitFlow` cho phép mã thông báo truy cập luồng ngầm và `oauth2AllowIdTokenImplicitFlow` cho phép mã thông báo ID luồng ngầm để người dùng đăng nhập.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-308', 'az-204', 308, 'You need to make sure that the solution meets the scaling requirements for Policy Service.  
  
Which Azure Application Insights data model should you use?', '[{"key":"A","text":"an Application Insights dependency"},{"key":"B","text":"an Application Insights event"},{"key":"C","text":"an Application Insights trace"},{"key":"D","text":"an Application Insights metric"}]'::jsonb, '["D"]'::jsonb, 'Số liệu Application Insights, bao gồm số liệu tùy chỉnh, cung cấp phép đo từ xa theo chuỗi thời gian bằng số mà Azure Monitor có thể sử dụng để đưa ra các quyết định tự động chia tỷ lệ. Số liệu tùy chỉnh có thể biểu thị số lượng hành động chính sách đang được thực hiện.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure Azure Disk Encryption for the VM. How should you complete the Azure CLI commands?', '[]'::jsonb, '[]'::jsonb, '`az keyvault create` tạo Key Vault và `az keyvault key create` tạo khóa được bảo vệ bằng phần mềm trong đó. `az vm create` cung cấp VM. Mã hóa đĩa Azure được bật với `az vm encryption enable`; `--volume-type all` mã hóa cả hệ điều hành và đĩa dữ liệu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Implement API Management policies to ensure images are processed correctly. Complete the inbound policy by selecting the appropriate options.', '[]'::jsonb, '[]'::jsonb, 'Đối với các yêu cầu dưới ngưỡng, `delete` sẽ xóa tiêu đề `x-large-request` do khách hàng cung cấp để một hình ảnh nhỏ không thể được chỉ định cấp dịch vụ hình ảnh lớn. `set-backend-service` chuyển hướng các yêu cầu còn lại đến phần phụ trợ hình ảnh lớn và `base-url` cung cấp URL của phần phụ trợ đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the manifest?', '[]'::jsonb, '[]'::jsonb, '`requiredResourceAccess` khai báo quyền Microsoft Graph được ủy quyền mà ứng dụng yêu cầu. `AzureADMultipleOrgs` giúp ứng dụng có nhiều đối tượng thuê cho các tài khoản Azure AD của tổ chức. Để hiển thị cài đặt tệp kê khai luồng tiềm ẩn SPA cũ, `oauth2AllowImplicitFlow` phải được bật (`true`).', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you use?', '[{"key":"A","text":"Azure AD access token"},{"key":"B","text":"Azure RBAC role"},{"key":"C","text":"Shared access signature (SAS) token"},{"key":"D","text":"Azure AD ID token"},{"key":"E","text":"Azure AD refresh token"}]'::jsonb, '["C"]'::jsonb, 'Mã thông báo chữ ký truy cập chung (SAS) cấp quyền truy cập được ủy quyền vào tài nguyên Bộ lưu trữ Azure với các quyền, phạm vi tài nguyên và thời hạn hiệu lực được xác định. Nó phù hợp để cung cấp quyền truy cập được kiểm soát bởi dịch vụ kiểm kê vào dữ liệu vị trí.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-315', 'az-204', 315, 'You have 100 Azure virtual machines (VMs) that have system-assigned managed identity enabled.  
  
You need to determine the object ID attribute value for each identity.  
  
Which command should you use?', '[{"key":"A","text":"az ad sp credential list"},{"key":"B","text":"Get-AzVM"},{"key":"C","text":"Get-AzureADUser"},{"key":"D","text":"az resource show"}]'::jsonb, '["D"]'::jsonb, 'ID đối tượng của system-assigned managed identity là `principalId` trong thuộc tính `identity` của tài nguyên Azure. `az resource show` có thể truy xuất tài nguyên VM và truy vấn `identity.principalId`, làm cho nó phù hợp để lấy ID đối tượng của từng danh tính VM. Tài liệu của Microsoft truy xuất ID chính dịch vụ VM managed identity từ danh tính tài nguyên và sử dụng ID đó cho các nhiệm vụ truy cập.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure storage for each microservice. Each storage type can be used once, multiple times, or not at all.', '{"statements":[{"id":"serviceA","text":"serviceA","correctAnswer":"D"},{"id":"serviceB","text":"serviceB","correctAnswer":"C"},{"id":"serviceC","text":"serviceC","correctAnswer":"A"}],"choices":[{"key":"A","text":"Azure Blob Storage"},{"key":"B","text":"Azure Files storage"},{"key":"C","text":"Ephemeral volume"},{"key":"D","text":"Container file system"}]}'::jsonb, '["serviceA=D","serviceB=C","serviceC=A"]'::jsonb, 'Bộ lưu trữ hệ thống tệp vùng chứa chỉ hiển thị với các tiến trình trong vùng chứa hiện tại của nó và bị giới hạn ở dung lượng đĩa tạm thời của vùng chứa. Một ổ đĩa tạm thời có phạm vi bản sao, vẫn tồn tại sau khi khởi động lại vùng chứa trong bản sao đó và có thể được gắn bởi nhiều vùng chứa trong đó. Azure Blob Storage tồn tại độc lập với các bản sao và cung cấp ủy quyền cấp đối tượng thông qua Microsoft Entra ID/RBAC hoặc chữ ký truy cập được chia sẻ.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which consistency level should you use?', '[{"key":"A","text":"session"},{"key":"B","text":"eventual"},{"key":"C","text":"bounded staleness"},{"key":"D","text":"consistent prefix"}]'::jsonb, '["D"]'::jsonb, 'Tính nhất quán của tiền tố đảm bảo các lần đọc trả về tiền tố của chuỗi ghi đã cam kết, do đó việc ghi không bao giờ bị sai thứ tự. Nó sử dụng các lần đọc một bản sao, cung cấp khả năng đọc đồng thời cao hơn mức độ nhất quán yêu cầu số lần đọc đại biểu, chẳng hạn như độ cứng giới hạn.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which change feed processor components should you use? Each component may be used once, more than once, or not at all.', '{"statements":[{"id":"slot1","text":"Store the data from which the change feed is generated.","correctAnswer":"D"},{"id":"slot2","text":"Coordinate processing of the change feed across multiple workers.","correctAnswer":"C"},{"id":"slot3","text":"Use the change feed processor to listen for changes.","correctAnswer":"A"},{"id":"slot4","text":"Handle each batch of changes.","correctAnswer":"B"}],"choices":[{"key":"A","text":"Host"},{"key":"B","text":"Delegate"},{"key":"C","text":"Lease container"},{"key":"D","text":"Monitored container"}]}'::jsonb, '["slot1=D","slot2=C","slot3=A","slot4=B"]'::jsonb, 'Vùng chứa được giám sát là nguồn có các thay đổi được đọc. Bộ chứa cho thuê lưu trữ trạng thái cho thuê để các phiên bản bộ xử lý có thể phối hợp quyền sở hữu phân vùng. Một phiên bản máy chủ chạy bộ xử lý nguồn cấp dữ liệu thay đổi và lắng nghe những thay đổi mới, trong khi lệnh gọi lại đại biểu xử lý từng lô được phân phối.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which values should you use?', '[]'::jsonb, '[]'::jsonb, 'Azure Blob Storage hỗ trợ đồng thời lạc quan thông qua ETag. Một lần đọc sẽ trả về ETag của blob, giá trị này sẽ thay đổi sau khi ghi. Việc cung cấp giá trị đã lưu đó trong tiêu đề If-Match làm cho việc cập nhật có điều kiện trên blob vẫn có cùng ETag đó; nếu không thì Blob Storage trả về HTTP 412, yêu cầu ứng dụng đọc lại dữ liệu mới nhất trước khi cập nhật.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Configure the API Management caching policies.', '{"statements":[{"id":"caching_type","text":"Slot 1:","correctAnswer":"D"},{"id":"downstream_caching_type","text":"Slot 2:","correctAnswer":"C"},{"id":"vary_by_header","text":"Slot 3:","correctAnswer":"F"}],"choices":[{"key":"A","text":"Expect"},{"key":"B","text":"Public"},{"key":"C","text":"Private"},{"key":"D","text":"Internal"},{"key":"E","text":"External"},{"key":"F","text":"Authorization"}]}'::jsonb, '["caching_type=D","downstream_caching_type=C","vary_by_header=F"]'::jsonb, 'Tầng tiêu thụ sử dụng bộ nhớ đệm Quản lý API (nội bộ) tích hợp sẵn. Việc đánh dấu bộ nhớ đệm xuôi dòng là riêng tư sẽ ngăn các bộ nhớ đệm xuôi dòng được chia sẻ phân phát phản hồi của khách hàng này cho khách hàng khác. Việc thay đổi cách tra cứu bộ nhớ đệm theo tiêu đề Ủy quyền sẽ tạo ra các mục nhập bộ nhớ đệm riêng biệt cho các yêu cầu được xác thực bằng OAuth với các thông tin xác thực khác nhau.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which header should you specify?', '[{"key":"A","text":"Accept"},{"key":"B","text":"Content-Type"},{"key":"C","text":"If-Match"},{"key":"D","text":"Prefer"}]'::jsonb, '["D"]'::jsonb, 'Microsoft Graph sử dụng tiêu đề yêu cầu `Prefer` với giá trị `include-unknown-enum-members` để chọn tham gia vào các thành viên tương lai của bảng liệt kê có thể phát triển trong các hoạt động GET.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-324', 'az-204', 324, 'You have 100 Azure virtual machines (VMs) that have system-assigned managed identity enabled.  
  
You need to determine the value of the object ID attribute for each identity.  
  
Which command should you use?', '[{"key":"A","text":"Get-AzVM"},{"key":"B","text":"Get-AzureADUserOwnedObject"},{"key":"C","text":"az ad sp credential list"},{"key":"D","text":"az resource show"}]'::jsonb, '["A"]'::jsonb, 'system-assigned managed identity của máy ảo hiển thị ID đối tượng Microsoft Entra của nó là `Identity.PrincipalId`. `Get-AzVM` truy xuất các thuộc tính VM, do đó đầu ra của nó có thể được liệt kê để thu được `Identity.PrincipalId` cho mọi VM.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which eviction policy should you use?', '[{"key":"A","text":"allkeys-Iru"},{"key":"B","text":"volatile-Iru"},{"key":"C","text":"allkeys-lfu"},{"key":"D","text":"volatile-ttl"}]'::jsonb, '["A"]'::jsonb, 'Chính sách `allkeys-lru` loại bỏ khóa ít được sử dụng gần đây nhất khỏi toàn bộ bộ đệm, giữ lại các mục nhập được lưu trong bộ nhớ đệm cho những người chơi đã di chuyển gần đây nhất đồng thời cho phép xóa các mục nhập cũ, bao gồm cả những mục nhập dành cho người chơi đã đăng xuất. Microsoft ghi lại rằng `allkeys-lru` loại bỏ khóa ít được sử dụng gần đây nhất, trong khi `volatile-lru` chỉ xem xét các khóa có thời hạn sử dụng.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to configure the filtering.', '[]'::jsonb, '[]'::jsonb, 'Bộ lọc SQL có thể đánh giá các thuộc tính hệ thống Azure Service Bus. Hành động quy tắc SQL của nó chạy sau khi thông báo khớp và có thể thêm, xóa hoặc thay thế các thuộc tính; những thay đổi chỉ áp dụng cho bản sao tin nhắn được gửi tới thuê bao. Nội dung thư không được cập nhật bởi các hành động quy tắc và thông báo chủ đề ban đầu không thay đổi.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to ensure that conflicts return the correct response. Complete the policy. Each code segment may be used once, more than once, or not at all.', '{"statements":[{"id":"section_open","text":"Slot 1:","correctAnswer":"C"},{"id":"response_context","text":"Slot 2:","correctAnswer":"B"},{"id":"error_context","text":"Slot 3:","correctAnswer":"B"},{"id":"status_policy","text":"Slot 4:","correctAnswer":"D"},{"id":"section_close","text":"Slot 5:","correctAnswer":"C"}],"choices":[{"key":"A","text":"server"},{"key":"B","text":"context"},{"key":"C","text":"on-error"},{"key":"D","text":"set-status"},{"key":"E","text":"when-error"},{"key":"F","text":"override-status"}]}'::jsonb, '["section_open=C","response_context=B","error_context=B","status_policy=D","section_close=C"]'::jsonb, 'Phần `on-error` xử lý các lỗi xử lý chính sách và hiển thị chi tiết lỗi thông qua `context.LastError`. Điều kiện phát hiện phản hồi 500 sai và thông báo xung đột. `return-response` dừng xử lý đường ống và con `set-status` của nó đặt mã HTTP được trả về thành phản hồi xung đột 409 được yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the code? Each code segment may be used once, more than once, or not at all.', '{"statements":[{"id":"blank1","text":"Slot 1:","correctAnswer":"B"},{"id":"blank2","text":"Slot 2:","correctAnswer":"E"}],"choices":[{"key":"A","text":"topic"},{"key":"B","text":"status"},{"key":"C","text":"eventType"},{"key":"D","text":"Succeeded"},{"key":"E","text":"operationName"},{"key":"F","text":"resourceProvider"}]}'::jsonb, '["blank1=B","blank2=E"]'::jsonb, 'Trạng thái dữ liệu sự kiện xác định liệu thao tác có thành công hay không, do đó `status` phải được so sánh với `Succeeded`. Hoạt động tài nguyên được xác định bởi `operationName`, được so sánh với `Microsoft.Web/sites/write`; điều này nắm bắt các hoạt động ghi thành công cho các trang Azure App Service.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-330', 'az-204', 330, 'You need to implement the Azure Function for delivery driver profile information. Which configurations should you use?', '[]'::jsonb, '[]'::jsonb, 'MSAL mua lại mã thông báo nền tảng nhận dạng của Microsoft cho các API web được bảo vệ. Microsoft Graph cung cấp tài nguyên người dùng và các thuộc tính hồ sơ của nó. Azure Key Vault quản lý bí mật, khóa và chứng chỉ chứ không phải hồ sơ thư mục; Đồ thị Azure AD đã ngừng hoạt động.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which configuration should you use?', '[]'::jsonb, '[]'::jsonb, 'Miền sự kiện Lưới sự kiện cung cấp một điểm cuối xuất bản duy nhất cho nhiều chủ đề riêng lẻ và hỗ trợ xác thực và ủy quyền chi tiết cho mỗi chủ đề, cho phép phân vùng đối tượng thuê. Đăng ký sự kiện xác định điểm cuối đích và có thể lọc các sự kiện được phân phối theo loại sự kiện, do đó, nó chỉ định tuyến các sự kiện cần thiết đến ứng dụng Azure Functions.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
You need to store the templates in Azure. Complete the command.', '[]'::jsonb, '[]'::jsonb, 'Thông số mẫu Azure là các tài nguyên mẫu ARM được phiên bản sử dụng Azure RBAC. `az ts create` tạo thông số mẫu và tệp mẫu của nó phải là mẫu gốc, `mainTemplate.json`; các mẫu được liên kết được giải quyết từ mẫu chính đó.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which Azure Durable Functions type should you use?', '[{"key":"A","text":"orchestrator"},{"key":"B","text":"client"},{"key":"C","text":"entity"},{"key":"D","text":"activity"}]'::jsonb, '["A"]'::jsonb, 'Chức năng điều phối phối hợp các chức năng khác dưới dạng quy trình làm việc dựa trên mã, bền vững, cho phép các hành động được chạy theo trình tự xác định trong khi vẫn duy trì tiến trình trong thời gian chờ đợi kéo dài. Các phiên bản điều phối có thể chạy trong nhiều ngày hoặc lâu hơn, hỗ trợ quy trình kiểm tra tín dụng có thể mất nhiều ngày.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the trigger?', '[]'::jsonb, '[]'::jsonb, 'Trình kích hoạt trước Cosmos DB truy cập mục đang chờ xử lý thông qua `getContext().getRequest()`. Nó có thể thêm `tip` số mặc định của `0` cho các giá trị bị thiếu, null hoặc không phải số, sau đó sử dụng `setBody()` để ghi mục đã sửa đổi vào thao tác đang chờ xử lý.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which parameter should you use?', '[{"key":"A","text":"tenant"},{"key":"B","text":"code_challenge"},{"key":"C","text":"state"},{"key":"D","text":"client_id"},{"key":"E","text":"scope"}]'::jsonb, '["E"]'::jsonb, 'Tham số `scope` yêu cầu các quyền được ủy quyền mà ứng dụng web cần để truy cập tài nguyên thay mặt cho người dùng đã đăng nhập, bao gồm các quyền như `Calendars.Read` và `Mail.Send`.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
How should you complete the YAML configuration?', '[]'::jsonb, '[]'::jsonb, 'StorageClass xác định việc cung cấp khối lượng động. Azure Disk cung cấp bộ lưu trữ khối liên tục phù hợp với khối lượng công việc có độ trễ thấp; Premium_LRS nhắm mục tiêu hơn nữa đến hiệu suất được hỗ trợ bởi SSD. Đặt chính sách thu hồi thành `retain` sẽ bảo toàn đĩa cơ bản và dữ liệu của nó khi xác nhận quyền sở hữu liên quan bị xóa.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Identify how to reference the queue and blob names in the `function.json` file.', '[]'::jsonb, '[]'::jsonb, 'Azure Functions giải quyết tham chiếu cài đặt ứng dụng được đính kèm trong các dấu phần trăm, do đó `%input_queue%` cung cấp tên hàng đợi. Đối với trình kích hoạt Hàng đợi Azure, biểu thức liên kết `{queueTrigger}` đại diện cho nội dung thư; sử dụng nó trong đường dẫn liên kết blob sẽ làm cho tên blob bằng nội dung thư.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
What should you do?', '[{"key":"A","text":"Enable single revision mode."},{"key":"B","text":"Use a separate environment for each container."},{"key":"C","text":"Use a private container registry image and single image for all containers."},{"key":"D","text":"Use a single environment for all containers."},{"key":"E","text":"Enable multiple revision mode."}]'::jsonb, '["D"]'::jsonb, 'Môi trường Azure Container Apps cung cấp ranh giới chung cho các ứng dụng vùng chứa của nó. Các ứng dụng vùng chứa được triển khai trong cùng một môi trường sẽ chia sẻ mạng ảo và sử dụng cùng đích ghi nhật ký Log Analytics, do đó, một môi trường duy nhất đáp ứng cả hai yêu cầu.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Assign the value of the hazard message SessionID property to the ReplyToSessionId property."},{"key":"B","text":"Assign the value of the hazard message MessageId property to the DevileryCount property."},{"key":"C","text":"Assign the value of the hazard message SessionID property to the SequenceNumber property."},{"key":"D","text":"Assign the value of the hazard message MessageId property to the CorrelationId property."},{"key":"E","text":"Assign the value of the hazard message SequenceNumber property to the DeliveryCount property."},{"key":"F","text":"Assign the value of the hazard message MessageId property to the SequenceNumber property."}]'::jsonb, '["A","D"]'::jsonb, 'Định tuyến trả lời Azure Service Bus có thể sử dụng `ReplyToSessionId` để chỉ định mã định danh phiên cho các phản hồi. Một thư trả lời tương ứng với thông báo kích hoạt của nó bằng cách đặt `CorrelationId` của tin nhắn trả lời thành `MessageId` của tin nhắn được xử lý. Điều này bảo tồn dấu vết kiểm tra giữa tín hiệu nguy hiểm ban đầu và phản hồi của từng bộ điều khiển.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which SDK features should you use?', '[]'::jsonb, '[]'::jsonb, 'Phần bù là vị trí (con trỏ phía máy khách) của một sự kiện trong chuỗi sự kiện của phân vùng, do đó, nó điền vào chỗ trống1, yêu cầu cơ chế xác định vị trí trong chuỗi sự kiện. Điểm kiểm tra được Microsoft định nghĩa là quá trình mà người đọc đánh dấu hoặc cam kết vị trí của họ trong chuỗi sự kiện phân vùng, khớp với Blank2 gần như nguyên văn và là điều cho phép bộ xử lý sự kiện còn sót lại tiếp tục từ điểm chính xác nơi bộ xử lý sự kiện bị lỗi đã dừng. Hai danh sách thả xuống yêu cầu các câu trả lời khác nhau: phần bù là giá trị vị trí, trong khi điểm kiểm tra là cam kết lâu dài của phần bù đó với bộ lưu trữ cho phép khôi phục và cân bằng lại tải, do đó, câu trả lời Điểm kiểm tra cho cả hai đều không chính xác.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
NOTE: Each correct selection is worth one point.', '[{"key":"A","text":"Create and configure a new Azure Event Grid instance."},{"key":"B","text":"Configure Azure Key Vault alerts."},{"key":"C","text":"Create and assign an Azure Key Vault access policy."},{"key":"D","text":"Create and configure a key rotation policy during key creation."}]'::jsonb, '["A","D"]'::jsonb, 'Chính sách xoay vòng khóa Azure Key Vault có thể lên lịch tự động tạo các phiên bản khóa mới theo một khoảng thời gian xác định và vẫn cho phép xoay vòng theo yêu cầu. Hành động thông báo của nó kích hoạt các sự kiện khóa gần hết hạn Azure Event Grid trước khi hết hạn; định cấu hình Lưới sự kiện cho phép các sự kiện đó được chuyển đến trình xử lý thông báo.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
...
Which command should you use?', '[]'::jsonb, '[]'::jsonb, 'Các điều kiện cảnh báo số liệu sử dụng tên tổng hợp và số liệu, do đó `avg Percentage CPU > 85` đánh giá phần trăm CPU trung bình. `--window-size 5m` định cấu hình cửa sổ tổng hợp năm phút. `--evaluation-frequency` chỉ là nhịp mà quy tắc được đánh giá.', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published'),
  ('az204-349', 'az-204', 349, 'You need to access data from the user claim object in the e-commerce web app.  
  
What should you do first?', '[{"key":"A","text":"Write custom code to make a Microsoft Graph API call from the e-commerce web app."},{"key":"B","text":"Assign the Contributor RBAC role to the e-commerce web app by using the Resource Manager create role assignment API."},{"key":"C","text":"Update the e-commerce web app to read the HTTP request header values."},{"key":"D","text":"Using the Azure CLI, enable Cross-origin resource sharing (CORS) from the e-commerce checkout API to the e-commerce web app."}]'::jsonb, '["C"]'::jsonb, 'Xác thực Azure App Service hiển thị các khiếu nại của người dùng đã được xác thực đối với mã ứng dụng trong các tiêu đề yêu cầu HTTP được chèn vào, chẳng hạn như `X-MS-CLIENT-PRINCIPAL`. Ứng dụng phải đọc các tiêu đề đó để lấy và xử lý dữ liệu xác nhận quyền sở hữu. [Work with user identities in Azure App Service authentication](https://learn.microsoft.com/en-us/azure/app-service/configure-authentication-user-identities)', 'General', '["AZ-204","General"]'::jsonb, NULL, 'published')
ON CONFLICT (id) DO UPDATE SET
  text = EXCLUDED.text,
