...
const raw = JSON.parse(fs.readFileSync(path.join(__dirname, 'raw_ai200_data.json'), 'utf8'));
...
const convertedQuestions = raw.map((item) => {
  ...
  return {
    ...
    correctAnswers,
    explanation: viCache[`ai200-${qNum}`] || cleanText(item.finalExplanation),
    category: item.category,
    tags: ['AI-200', item.category],
    imageUrl,
    sourceTitle: `Examcademy AI-200 Question ${qNum}`,
    sourceUrl: item.url,
...
fs.writeFileSync(path.join(__dirname, '../src/data/ai200Questions.ts'), tsContent);
console.log('Generated src/data/ai200Questions.ts');

// 2. Output JSON import file
fs.writeFileSync(
  path.join(__dirname, '../src/data/ai200_questions_import.json'),
  JSON.stringify(convertedQuestions, null, 2)
);
console.log('Generated src/data/ai200_questions_import.json');
...
const sqlLines = [
  `-- AI-200 Questions Upsert SQL Script`,
...
const valueRows = convertedQuestions.map((q) => {
  ...
  return `  (${escapeSql(q.id)}, 'ai-200', ${q.questionNumber}, ${escapeSql(q.text)}, ${escapeSql(q.questionType)}, ${optionsJson}::jsonb, ${choicesJson}::jsonb, ${statementsJson}::jsonb, ${correctAnsJson}::jsonb, ${escapeSql(q.explanation)}, ${escapeSql(q.category)}, ${tagsJson}::jsonb, ${q.imageUrl ? escapeSql(q.imageUrl) : 'NULL'}, ${escapeSql(q.sourceTitle)}, ${escapeSql(q.sourceUrl)}, 'published')`;
});

sqlLines.push(valueRows.join(',\n') + ';');
sqlLines.push(``);

fs.writeFileSync(path.join(__dirname, '../src/data/ai200_questions_upsert.sql'), sqlLines.join('\n'));
console.log('Generated src/data/ai200_questions_upsert.sql');
